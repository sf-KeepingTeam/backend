...
public class ChargeBonusControllerTest extends MySqlTestContainerConfig {
  ...
  @Autowired ChargeBonusRepository chargeBonusRepository;
  ...
  private ChargeBonus createTestChargeBonus(
      Store store, Long chargeAmount, Integer bonusPercentage) {
    return chargeBonusRepository.save(
        ChargeBonus.builder()
            .store(store)
  ...
  class CreateChargeBonusTest {
    ...
    void createChargeBonus_success() throws Exception {
      ...
      mockMvc
          .perform(
              post("/owners/stores/{storeId}/charge-bonus", store.getStoreId())
    ...
    void createChargeBonus_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/owners/stores/{storeId}/charge-bonus", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
  ...
  class GetChargeBonusListTest {
    ...
    void getChargeBonusList_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/owners/stores/{storeId}/charge-bonus", store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
    ...
    void getChargeBonusList_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/owners/stores/{storeId}/charge-bonus", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetChargeBonusDetailTest {
    ...
    void getChargeBonusDetail_success() throws Exception {
      ...
      mockMvc
          .perform(
              get(
                      "/owners/stores/{storeId}/charge-bonus/{chargeBonusId}",
                      store.getStoreId(),
                      chargeBonus.getChargeBonusId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getChargeBonusDetail_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/owners/stores/{storeId}/charge-bonus/{chargeBonusId}", 1L, 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class UpdateChargeBonusTest {
    ...
    void updateChargeBonus_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              put("/owners/stores/{storeId}/charge-bonus/{chargeBonusId}", 1L, 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
  ...
  class DeleteChargeBonusTest {
    ...
    void deleteChargeBonus_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(delete("/owners/stores/{storeId}/charge-bonus/{chargeBonusId}", 1L, 1L))
          .andExpect(status().isUnauthorized());
    }
  }
}
