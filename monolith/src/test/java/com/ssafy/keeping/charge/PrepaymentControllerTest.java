...
public class PrepaymentControllerTest extends MySqlTestContainerConfig {
  ...
  class ReservePrepaymentTest {
    ...
    void reservePrepayment_success() throws Exception {
      ...
      PrepaymentReserveRequest request =
          PrepaymentReserveRequest.builder().amount(10000L).orderName("테스트 충전").build();

      // when & then
      mockMvc
          .perform(
              post("/api/v1/stores/{storeId}/prepayment/reserve", store.getStoreId())
    ...
    void reservePrepayment_validationFail_nullAmount() throws Exception {
      ...
      Store store = createTestStore(owner);
      Authentication auth = createAuth(customer.getCustomerId());
      PrepaymentReserveRequest request = PrepaymentReserveRequest.builder().amount(null).build();
    ...
    void reservePrepayment_validationFail_negativeAmount() throws Exception {
      ...
      Store store = createTestStore(owner);
      Authentication auth = createAuth(customer.getCustomerId());
      PrepaymentReserveRequest request = PrepaymentReserveRequest.builder().amount(-1000L).build();
    ...
    void reservePrepayment_unauthorized() throws Exception {
      // given
      PrepaymentReserveRequest request = PrepaymentReserveRequest.builder().amount(10000L).build();

      // when & then
      mockMvc
          .perform(
              post("/api/v1/stores/{storeId}/prepayment/reserve", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
  ...
  class ConfirmPrepaymentTest {
    ...
    void confirmPrepayment_validationFail_missingPaymentKey() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/stores/{storeId}/prepayment/confirm", store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isBadRequest());
    ...
    void confirmPrepayment_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/stores/{storeId}/prepayment/confirm", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
