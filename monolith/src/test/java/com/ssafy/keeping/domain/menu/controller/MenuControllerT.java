...
class MenuControllerT extends MySqlTestContainerConfig {
  ...
  class GetAllMenus {
    ...
    void success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/stores/{storeId}/menus", store.getStoreId())
          ...
          // 아메리카노 검증
          .andExpect(
              jsonPath("$.data[?(@.menuName == '아메리카노')].menuId")
                  .value(hasItem(americano.getMenuId().intValue())))
          ...
          .andExpect(
              jsonPath("$.data[?(@.menuName == '치즈케이크')].categoryName").value(hasItem("디저트")));
    }
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              get("/stores/{storeId}/menus", 999999L).with(customer(testCustomer.getCustomerId())))
  ...
  class GetMenusByCategory {
    ...
    void success() throws Exception {
      ...
      mockMvc
          .perform(
              get(
                      "/stores/{storeId}/menus/categories/{categoryId}",
                      store.getStoreId(),
                      coffeeCategory.getCategoryId())
                  .with(customer(testCustomer.getCustomerId())))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success").value(true))
          .andExpect(jsonPath("$.status").value(200))
    ...
    void success_categoryNotFound_emptyArray() throws Exception {
      ...
      mockMvc
          .perform(
              get("/stores/{storeId}/menus/categories/{categoryId}", store.getStoreId(), 999999L)
                  .with(customer(testCustomer.getCustomerId())))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success").value(true))
          .andExpect(jsonPath("$.data").isArray())
