...
/**
 * CancelService saga 단위 테스트.
 * Phase A(CancelReclaimService)와 Phase B(TossPaymentClient)를 mock하여
 * saga 오케스트레이션 로직만 검증한다.
 */
...
class CancelServiceSagaTest {
    ...
    @Mock
    private CancelReclaimService cancelReclaimService;
    ...
    @BeforeEach
    void setUp() {
        normalRequest = CancelRequestDto.builder()
                .paymentKey("pk_test_123")
                .cancelReason("고객 변심")
                .build();

        reclaimResult = CancelReclaimResult.builder()
    ...
    void cancelPayment_alreadyCanceledPayment_treatedAsSuccess() {
        ...
        verify(cancelReclaimService).updateRefundStatus(100L, RefundStatus.REFUND_DONE);
        // 영구실패로 처리되지 않음을 검증
        verify(cancelReclaimService, never()).updateRefundStatus(eq(100L), eq(RefundStatus.REFUND_PERMANENT_FAILED));
        verify(tossPaymentClient, times(1)).cancelPayment(anyString(), any());
    }
