...
public class OwnerStoreControllerT extends MySqlTestContainerConfig {
  ...
  void setUp() {
    ...
    var owner =
        ownerRepository.save(
            OwnerFixtures.owner(AuthProvider.KAKAO, "kakao_" + UUID.randomUUID(), "점주1"));
    ownerId = owner.getOwnerId();

    var otherOwner =
        ownerRepository.save(
            OwnerFixtures.owner(AuthProvider.KAKAO, "kakao_" + UUID.randomUUID(), "점주2"));
    otherOwnerId = otherOwner.getOwnerId();

    var store =
        storeRepository.save(StoreFixtures.store(owner, "원래매장", StoreStatus.ACTIVE, "CAFE"));
    storeId = store.getStoreId();

    String dupTaxId = "123-45-67890";
    String dupAddress = "서울시 강남구 어딘가 1";
    storeRepository.save(
        StoreFixtures.storeWithTaxAndAddress(
  ...
  void createStore_success() throws Exception {
    MockMultipartFile imgFile =
        new MockMultipartFile("imgFile", "store.png", "image/png", "dummy".getBytes());

    mockMvc
        .perform(
            multipart("/owners/stores")
                .file(imgFile)
  ...
  void createStore_fail_duplicate() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.status").value(ErrorCode.STORE_ALREADY_EXISTS.getHttpStatus().value()))
  ...
  void editStore_success() throws Exception {
    MockMultipartFile newImg =
        new MockMultipartFile("imgFile", "new.png", "image/png", "dummy".getBytes());

    mockMvc
        .perform(
            multipart("/owners/stores/{storeId}", storeId)
                .file(newImg)
  ...
  void deleteStore_success_deleted() throws Exception {
    ...
    mockMvc
        .perform(delete("/owners/stores/{storeId}", storeId).with(owner(ownerId)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getMyStores_success() throws Exception {
    mockMvc
        .perform(get("/owners/stores").with(owner(ownerId)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
