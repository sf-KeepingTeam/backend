...
class StoreControllerIT extends MySqlTestContainerConfig {
  ...
  void setUp() {
    ...
    Store active1 =
        storeRepository.save(StoreFixtures.store(owner, "활성매장1", StoreStatus.ACTIVE, "CAFE"));
    ...
    Store inactive =
        storeRepository.save(StoreFixtures.store(owner, "삭제매장", StoreStatus.DELETED, "FOOD"));
  ...
  void getAllStore_success_onlyActiveReturned() throws Exception {
    ...
    mockMvc
        .perform(get("/stores").with(customer(customer.getCustomerId())))
  ...
  void getStore_success_active() throws Exception {
    mockMvc
        .perform(get("/stores/{storeId}", activeStoreId).with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getStore_fail_inactive() throws Exception {
    mockMvc
        .perform(get("/stores/{storeId}", inactiveStoreId).with(customer(customer.getCustomerId())))
  ...
  void getStore_fail_notExists() throws Exception {
    mockMvc
        .perform(get("/stores/{storeId}", 999999L).with(customer(customer.getCustomerId())))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.status").value(ErrorCode.STORE_NOT_FOUND.getHttpStatus().value()))
        .andExpect(jsonPath("$.message").value(ErrorCode.STORE_NOT_FOUND.getMessage()))
  ...
  void getAllStoreByCategory_success_food_onlyActiveFoodReturned() throws Exception {
    mockMvc
        .perform(get("/stores").param("category", "FOOD").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getAllStoreByCategory_success_cafe_onlyActiveCafeReturned() throws Exception {
    mockMvc
        .perform(get("/stores").param("category", "CAFE").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getAllStoreByCategory_blankCategory_returnsAllActive() throws Exception {
    mockMvc
        .perform(get("/stores").param("category", "   ").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getAllStoreByCategory_trimApplied() throws Exception {
    mockMvc
        .perform(
            get("/stores").param("category", "  FOOD  ").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data", hasSize(2)))
        .andExpect(jsonPath("$.data[*].category", everyItem(is("FOOD"))));
  }
  ...
  void getStoreByName_success_similarity_onlyActiveReturned() throws Exception {
    mockMvc
        .perform(get("/stores").param("name", "활성매장").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getStoreByName_blank_returnsAllActive() throws Exception {
    mockMvc
        .perform(get("/stores").param("name", "   ").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.data", hasSize(3)))
  ...
  void getStoreByName_success_exactOneMatched() throws Exception {
    mockMvc
        .perform(get("/stores").param("name", "활성매장1").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
        .andExpect(jsonPath("$.data", hasSize(1)))
  ...
  void getStoreByName_deletedExcluded() throws Exception {
    mockMvc
        .perform(get("/stores").param("name", "삭제매장").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.status").value(200))
  ...
  void getStoreByName_caseInsensitive() throws Exception {
    ...
    mockMvc
        .perform(get("/stores").param("name", "coffeek").with(customer(customer.getCustomerId())))
        .andExpect(status().isOk())
