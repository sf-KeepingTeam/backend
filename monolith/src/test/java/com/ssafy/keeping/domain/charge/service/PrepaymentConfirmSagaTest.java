...
/**
 * PrepaymentService saga 오케스트레이터 단위 테스트.
 * Phase A/B(PrepaymentConfirmService), 보상(PrepaymentCompensationService),
 * TossPaymentClient를 mock하여 오케스트레이션 로직만 검증한다.
 */
...
class PrepaymentConfirmSagaTest {
    ...
    @Mock private com.ssafy.keeping.domain.user.customer.repository.CustomerRepository customerRepository;
    @Mock private com.ssafy.keeping.domain.store.repository.StoreRepository storeRepository;
    @Mock private com.ssafy.keeping.domain.charge.repository.PaymentReservationRepository paymentReservationRepository;
    ...
    @BeforeEach
    void setUp() {
        confirmRequest = PrepaymentConfirmRequest.builder()
        ...
        prepareResult = ConfirmPrepareResult.builder()
                .reservationId(100L)
                .orderId("ORDER_TEST_001")
                .paymentKey("pk_test_123")
                .amount(50000L)
    ...
    void confirmPayment_success() {
        // given
        when(prepaymentConfirmService.prepareConfirm(eq(1L), eq(1L), any()))
        ...
        PrepaymentResponseDto creditResult = PrepaymentResponseDto.builder()
                .transactionId(200L)
        ...
        when(prepaymentConfirmService.creditPoints(
                eq("ORDER_TEST_001"), eq("pk_test_123"), eq(mockWallet), eq(mockStore), eq(50000L), any()))
                .thenReturn(creditResult);

        // when
        IdempotentResult<PrepaymentResponseDto> result = prepaymentService.confirmPayment(1L, 1L, confirmRequest);
        ...
        verify(prepaymentConfirmService).creditPoints(anyString(), anyString(), any(), any(), anyLong(), any());
    ...
    void confirmPayment_tossConfirmFails_reservationFailed() {
        ...
        verify(prepaymentConfirmService, never()).creditPoints(any(), any(), any(), any(), anyLong(), any());
        verify(prepaymentCompensationService, never()).compensate(anyLong(), anyString(), anyString());
    }

    @Test
    @DisplayName("케이스3: 적립 실패(Phase B) → compensate 호출(토스 cancel + 예약 FAILED) + PAYMENT_CONFIRM_FAILED 예외")
