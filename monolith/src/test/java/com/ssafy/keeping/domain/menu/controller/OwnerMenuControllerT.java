...
class OwnerMenuControllerT extends MySqlTestContainerConfig {
  ...
  void setUp() {
    ...
    store =
        storeRepository.save(StoreFixtures.store(testOwner, "테스트카페", StoreStatus.ACTIVE, "CAFE"));
    otherStore =
        storeRepository.save(StoreFixtures.store(otherOwner, "다른카페", StoreStatus.ACTIVE, "CAFE"));
  ...
  class CreateMenu {
    ...
    void success() throws Exception {
      MockMultipartFile imgFile =
          new MockMultipartFile("imgFile", "menu.png", "image/png", "dummy".getBytes());

      mockMvc
          .perform(
              multipart("/owners/stores/{storeId}/menus", store.getStoreId())
                  .file(imgFile)
    ...
    void fail_storeNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              multipart("/owners/stores/{storeId}/menus", 999999L)
                  .file(imgFile)
                  .param("menuName", "아메리카노")
                  .param("categoryId", coffeeCategory.getCategoryId().toString())
                  .param("price", "4500")
    ...
    void fail_categoryNotFound() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(
              jsonPath("$.status").value(ErrorCode.MENU_CATEGORY_NOT_FOUND.getHttpStatus().value()))
    ...
    void fail_categoryFromOtherStore() throws Exception {
      MenuCategory otherCategory =
          menuCategoryRepository.save(MenuCategoryFixtures.category(otherStore, "다른카테고리", 1));
    ...
    void fail_duplicateMenuName() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(
              jsonPath("$.status").value(ErrorCode.DUPLICATE_RESOURCE.getHttpStatus().value()))
    ...
    void fail_imageUploadError() throws Exception {
      ...
      when(imageService.uploadImage(any(MultipartFile.class), eq("menu"))).thenReturn(null);
      ...
      mockMvc
          ...
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(
              jsonPath("$.status").value(ErrorCode.IMAGE_UPLOAD_ERROR.getHttpStatus().value()))
  ...
  class EditMenu {
    ...
    void success() throws Exception {
      MockMultipartFile imgFile =
          new MockMultipartFile("imgFile", "new-menu.png", "image/png", "dummy".getBytes());

      mockMvc
          .perform(
              multipart(
                      "/owners/stores/{storeId}/menus/{menusId}",
                      store.getStoreId(),
                      existingMenu.getMenuId())
                  .file(imgFile)
    ...
    void fail_menuNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              multipart("/owners/stores/{storeId}/menus/{menusId}", store.getStoreId(), 999999L)
                  .file(imgFile)
                  .param("menuName", "수정시도")
                  .param("categoryId", coffeeCategory.getCategoryId().toString())
                  .param("price", "5000")
  ...
  class GetAllMenusForOwner {
    ...
    void success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/owners/stores/{storeId}/menus", store.getStoreId())
                  .with(owner(testOwner.getOwnerId())))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success").value(true))
          .andExpect(jsonPath("$.status").value(200))
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              get("/owners/stores/{storeId}/menus", 999999L).with(owner(testOwner.getOwnerId())))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(jsonPath("$.status").value(ErrorCode.STORE_NOT_FOUND.getHttpStatus().value()))
          .andExpect(jsonPath("$.message").value(ErrorCode.STORE_NOT_FOUND.getMessage()))
  ...
  class DeleteMenu {
    ...
    void fail_menuNotFound() throws Exception {
      mockMvc
          .perform(
              delete("/owners/stores/{storeId}/menus/{menusId}", store.getStoreId(), 999999L)
                  .with(owner(testOwner.getOwnerId())))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(jsonPath("$.status").value(ErrorCode.MENU_NOT_FOUND.getHttpStatus().value()))
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              delete("/owners/stores/{storeId}/menus/{menusId}", 999999L, existingMenu.getMenuId())
                  .with(owner(testOwner.getOwnerId())))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(jsonPath("$.status").value(ErrorCode.STORE_NOT_FOUND.getHttpStatus().value()))
  ...
  class DeleteAllMenus {
    ...
    void success() throws Exception {
      mockMvc
          .perform(
              delete("/owners/stores/{storeId}/menus", store.getStoreId())
                  .with(owner(testOwner.getOwnerId())))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success").value(true))
          .andExpect(jsonPath("$.status").value(200))
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              delete("/owners/stores/{storeId}/menus", 999999L).with(owner(testOwner.getOwnerId())))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(jsonPath("$.status").value(ErrorCode.STORE_NOT_FOUND.getHttpStatus().value()))
          .andExpect(jsonPath("$.data").doesNotExist());
