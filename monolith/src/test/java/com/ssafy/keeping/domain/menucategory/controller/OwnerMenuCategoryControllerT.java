...
class OwnerMenuCategoryControllerT extends MySqlTestContainerConfig {
  ...
  class CreateMenuCategory {
    ...
    void success_majorCategory() throws Exception {
      ...
      mockMvc
          .perform(
              post("/owners/stores/{storeId}/menus/categories", store.getStoreId())
    ...
    void fail_storeNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              post("/owners/stores/{storeId}/menus/categories", 999999L)
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
    ...
    void fail_parentFromOtherStore() throws Exception {
      ...
      MenuCategoryRequestDto request =
          new MenuCategoryRequestDto("에스프레소", otherCategory.getCategoryId());
  ...
  class GetAllMajorCategories {
    ...
    void success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/owners/stores/{storeId}/menus/categories", store.getStoreId())
                  .with(owner(testOwner.getOwnerId())))
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              get("/owners/stores/{storeId}/menus/categories", 999999L)
                  .with(owner(testOwner.getOwnerId())))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(jsonPath("$.status").value(ErrorCode.STORE_NOT_FOUND.getHttpStatus().value()))
  ...
  class EditMenuCategory {
    ...
    void success_changeToSubCategory() throws Exception {
      MenuCategory newParent =
          menuCategoryRepository.save(MenuCategoryFixtures.category(store, "음료", 2));

      MenuCategoryEditRequestDto request =
          new MenuCategoryEditRequestDto("커피", newParent.getCategoryId());
    ...
    void fail_parentFromOtherStore() throws Exception {
      ...
      MenuCategoryEditRequestDto request =
          new MenuCategoryEditRequestDto("커피", otherCategory.getCategoryId());
  ...
  class DeleteMenuCategory {
    ...
    void fail_hasChildren() throws Exception {
      ...
      menuCategoryRepository.save(
          MenuCategoryFixtures.subCategory(store, existingCategory, "에스프레소", 1));
      ...
      mockMvc
          ...
          .andExpect(
              jsonPath("$.status")
                  .value(ErrorCode.MENU_CATEGORY_HAS_CHILDREN.getHttpStatus().value()))
