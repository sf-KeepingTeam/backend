  ...
  class GetAllMajorCategories {
    ...
    void success() throws Exception {
      ...
      MenuCategory beverage =
          menuCategoryRepository.save(MenuCategoryFixtures.category(store, "음료", 3));

      mockMvc
          .perform(
              get("/stores/{storeId}/menus/categories", store.getStoreId())
          ...
          .andExpect(
              jsonPath("$.data[?(@.categoryName == '커피')].categoryId")
                  .value(org.hamcrest.Matchers.hasItem(coffee.getCategoryId().intValue())))
          .andExpect(
              jsonPath("$.data[?(@.categoryName == '커피')].displayOrder")
                  .value(org.hamcrest.Matchers.hasItem(1)))
          .andExpect(
              jsonPath("$.data[?(@.categoryName == '디저트')].categoryId")
                  .value(org.hamcrest.Matchers.hasItem(dessert.getCategoryId().intValue())))
          .andExpect(
              jsonPath("$.data[?(@.categoryName == '음료')].categoryId")
                  .value(org.hamcrest.Matchers.hasItem(beverage.getCategoryId().intValue())));
    }
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              get("/stores/{storeId}/menus/categories", 999999L)
                  .with(customer(testCustomer.getCustomerId())))
