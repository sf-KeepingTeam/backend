...
class StoreStatisticsControllerT extends MySqlTestContainerConfig {
  ...
  void setUp() {
    ...
    testOwner =
        ownerRepository.save(
            OwnerFixtures.owner(AuthProvider.KAKAO, "kakao_" + UUID.randomUUID(), "테스트점주"));
    otherOwner =
        ownerRepository.save(
            OwnerFixtures.owner(AuthProvider.KAKAO, "kakao_" + UUID.randomUUID(), "다른점주"));

    testStore =
        storeRepository.save(StoreFixtures.store(testOwner, "테스트매장", StoreStatus.ACTIVE, "CAFE"));
  ...
  /**
   * Transaction 저장 후 createdAt을 원하는 시간으로 직접 UPDATE @CreationTimestamp가 persist 시점에 현재 시간으로 덮어쓰기
   * 때문에, JdbcTemplate으로 직접 UPDATE하여 테스트에서 원하는 날짜를 설정함
   */
  private Transaction saveTransactionWithCreatedAt(
      Transaction transaction, LocalDateTime createdAt) {
   ...
   * 테스트용 거래 데이터 생성
   *
   * <p>오늘: CHARGE 10000, USE 5000 어제: CHARGE 20000, USE 8000 지난주: CHARGE 15000 지난달: CHARGE 30000,
   * USE 12000
   *
   * <p>전체 통계 기준: - 총 충전: 75000 (4건) - 총 사용: 25000 (3건) - 총 거래: 7건
   *
   * <p>지난주~오늘 (7일) 기준: - 충전: 45000 (3건) - 오늘 10000 + 어제 20000 + 지난주 15000 - 사용: 13000 (2건) - 오늘
   * 5000 + 어제 8000 - 총 거래: 5건
   */
  private void createTestTransactions() {
    ...
    saveTransactionWithCreatedAt(
        TransactionFixtures.charge(wallet, customer, testStore, 10000L), today);
    saveTransactionWithCreatedAt(
        TransactionFixtures.use(wallet, customer, testStore, 5000L), today);

    // 어제 거래: CHARGE 20000, USE 8000
    saveTransactionWithCreatedAt(
        TransactionFixtures.charge(wallet, customer, testStore, 20000L), yesterday);
    saveTransactionWithCreatedAt(
        TransactionFixtures.use(wallet, customer, testStore, 8000L), yesterday);

    // 지난주 거래: CHARGE 15000
    saveTransactionWithCreatedAt(
        TransactionFixtures.charge(wallet, customer, testStore, 15000L), lastWeek);

    // 지난달 거래: CHARGE 30000, USE 12000
    saveTransactionWithCreatedAt(
        TransactionFixtures.charge(wallet, customer, testStore, 30000L), lastMonth);
    saveTransactionWithCreatedAt(
        TransactionFixtures.use(wallet, customer, testStore, 12000L), lastMonth);
  }
  ...
  class GetOverallStatistics {
    ...
    void success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/stores/{storeId}/statistics/overall", testStore.getStoreId())
    ...
    void fail_unauthorized() throws Exception {
      mockMvc
          ...
          .andExpect(
              jsonPath("$.status").value(ErrorCode.UNAUTHORIZED_ACCESS.getHttpStatus().value()))
    ...
    void fail_storeNotFound() throws Exception {
      mockMvc
          .perform(
              get("/stores/{storeId}/statistics/overall", 999999L)
                  .with(owner(testOwner.getOwnerId())))
  ...
  class GetDailyStatistics {
    ...
    void success_withDate() throws Exception {
      ...
      StatisticsRequestDto request = StatisticsRequestDto.builder().date(today).build();

      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/daily", testStore.getStoreId())
                  .with(owner(testOwner.getOwnerId()))
    ...
    void success_noTransactionsOnDate() throws Exception {
      ...
      StatisticsRequestDto request = StatisticsRequestDto.builder().date(noTransactionDate).build();
    ...
    @DisplayName("실패: 다른 점주가 조회 시도 - UNAUTHORIZED_ACCESS")
    void fail_unauthorized() throws Exception {
      StatisticsRequestDto request = StatisticsRequestDto.builder().date(LocalDate.now()).build();
    ...
    void fail_storeNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/daily", 999999L)
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isNotFound())
  ...
  class GetPeriodStatistics {
    ...
    void success() throws Exception {
      ...
      StatisticsRequestDto request =
          StatisticsRequestDto.builder().startDate(startDate).endDate(endDate).build();

      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/period", testStore.getStoreId())
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void fail_nullStartDate() throws Exception {
      StatisticsRequestDto request =
          StatisticsRequestDto.builder().endDate(LocalDate.now()).build();
      ...
      mockMvc
          ...
          .andExpect(jsonPath("$.success").value(false))
          .andExpect(
              jsonPath("$.status").value(ErrorCode.INVALID_DATE_RANGE.getHttpStatus().value()))
    ...
    void fail_nullEndDate() throws Exception {
      StatisticsRequestDto request =
          StatisticsRequestDto.builder().startDate(LocalDate.now().minusWeeks(1)).build();
    ...
    void fail_storeNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/period", 999999L)
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isNotFound())
  ...
  class GetMonthlyStatistics {
    ...
    void success_withDate() throws Exception {
      ...
      StatisticsRequestDto request = StatisticsRequestDto.builder().date(thisMonth).build();

      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/monthly", testStore.getStoreId())
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void success_noTransactionsInMonth() throws Exception {
      ...
      StatisticsRequestDto request =
          StatisticsRequestDto.builder().date(noTransactionMonth).build();
    ...
    void fail_storeNotFound() throws Exception {
      ...
      mockMvc
          .perform(
              post("/stores/{storeId}/statistics/monthly", 999999L)
                  .with(owner(testOwner.getOwnerId()))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isNotFound())
