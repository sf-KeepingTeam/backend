...
public final class TestAuth {
  ...
  public static RequestPostProcessor customer(Long customerId) {
    ...
    var token =
        new UsernamePasswordAuthenticationToken(
            principal, "N/A", List.of(new SimpleGrantedAuthority("ROLE_CUSTOMER")));
  ...
  public static RequestPostProcessor owner(Long ownerId) {
    ...
    var token =
        new UsernamePasswordAuthenticationToken(
            principal, "N/A", List.of(new SimpleGrantedAuthority("ROLE_OWNER")));
    return authentication(token);
  }

  /** 인증이 필요 없는 엔드포인트 테스트용(명시적으로 "anonymous" 느낌) 필요 없으면 삭제해도 됨. */
  public static RequestPostProcessor anonymous() {
    var token =
        new UsernamePasswordAuthenticationToken(
            "anonymous", "N/A", List.of(new SimpleGrantedAuthority("ROLE_ANONYMOUS")));
    return authentication(token);
  }
}
