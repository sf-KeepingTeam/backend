...
public class WalletControllerTest extends MySqlTestContainerConfig {
  ...
  @Autowired WalletStoreBalanceRepository walletStoreBalanceRepository;

  @Autowired WalletStoreLotRepository walletStoreLotRepository;
  ...
  private Wallet createIndividualWallet(Customer customer) {
    return walletRepository.save(
        Wallet.builder().customer(customer).walletType(WalletType.INDIVIDUAL).build());
  }
  ...
  class GetGroupWalletsTest {
    ...
    void getGroupWallets_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/wallets/groups/{groupId}", group.getGroupId())
    ...
    void getGroupWallets_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(get("/wallets/groups/{groupId}", 1L)).andExpect(status().isUnauthorized());
    }
  }
  ...
  class SharePointsTest {
    ...
    void sharePoints_success() throws Exception {
      ...
      // 개인 지갑에 잔액 생성
      WalletStoreBalance individualBalance =
      ...
      mockMvc
          .perform(
              post(
                      "/wallets/groups/{groupId}/stores/{storeId}",
                      group.getGroupId(),
                      store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
    ...
    void sharePoints_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/wallets/groups/{groupId}/stores/{storeId}", 1L, 1L)
                  .header("Idempotency-Key", UUID.randomUUID().toString())
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
  ...
  class GetReclaimablePointsTest {
    ...
    void getReclaimablePoints_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/wallets/{walletId}/stores/{storeId}/points/available", 1L, 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class ReclaimPointsTest {
    ...
    void reclaimPoints_success() throws Exception {
      ...
      walletStoreBalanceRepository.save(
          WalletStoreBalance.builder().wallet(groupWallet).store(store).balance(3000L).build());
      ...
      mockMvc
          .perform(
              post(
                      "/wallets/groups/{groupId}/stores/{storeId}/reclaim",
                      group.getGroupId(),
                      store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .header("Idempotency-Key", UUID.randomUUID().toString())
    ...
    void reclaimPoints_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/wallets/groups/{groupId}/stores/{storeId}/reclaim", 1L, 1L)
                  .header("Idempotency-Key", UUID.randomUUID().toString())
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
  ...
  class GetPersonalWalletBalanceTest {
    ...
    void getPersonalWalletBalance_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(get("/wallets/individual/balance")).andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetGroupWalletBalanceTest {
    ...
    void getGroupWalletBalance_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/wallets/groups/{groupId}/balance", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getGroupWalletBalance_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/wallets/groups/{groupId}/balance", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetPersonalWalletStoreDetailTest {
    ...
    void getPersonalWalletStoreDetail_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/wallets/individual/stores/{storeId}/detail", store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getPersonalWalletStoreDetail_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/wallets/individual/stores/{storeId}/detail", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetGroupWalletStoreDetailTest {
    ...
    void getGroupWalletStoreDetail_success() throws Exception {
      ...
      mockMvc
          .perform(
              get(
                      "/wallets/groups/{groupId}/stores/{storeId}/detail",
                      group.getGroupId(),
                      store.getStoreId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getGroupWalletStoreDetail_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/wallets/groups/{groupId}/stores/{storeId}/detail", 1L, 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetBothWalletBalanceTest {
    ...
    void getBothWalletBalance_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(get("/wallets/both/balance")).andExpect(status().isUnauthorized());
    }
  }
}
