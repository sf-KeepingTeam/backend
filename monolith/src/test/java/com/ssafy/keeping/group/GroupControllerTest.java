...
public class GroupControllerTest extends MySqlTestContainerConfig {
  ...
  @Autowired GroupAddRequestRepository groupAddRequestRepository;
  ...
  private GroupMember addMemberToGroup(Group group, Customer customer, boolean isLeader) {
    return groupMemberRepository.save(
        GroupMember.builder().group(group).user(customer).leader(isLeader).build());
  }

  private GroupAddRequest createTestAddRequest(Group group, Customer customer) {
    return groupAddRequestRepository.save(
  ...
  private Wallet createGroupWallet(Group group) {
    return walletRepository.save(
        Wallet.builder().group(group).walletType(WalletType.GROUP).build());
  }
  ...
  class SearchGroupTest {
    ...
    void searchGroup_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(get("/groups").param("name", "테스트")).andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetGroupTest {
    ...
    void getGroup_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/groups/{groupId}", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.message").value("해당 모임이 조회되었습니다."));
    }
    ...
    void getGroup_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(get("/groups/{groupId}", 1L)).andExpect(status().isUnauthorized());
    }
  }
  ...
  class EditGroupTest {
    ...
    void editGroup_success() throws Exception {
      ...
      mockMvc
          .perform(
              patch("/groups/{groupId}", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
  ...
  class GetGroupMembersTest {
    ...
    void getGroupMembers_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/groups/{groupId}/group-members", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getGroupMembers_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/groups/{groupId}/group-members", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class CreateAddRequestTest {
    ...
    void createAddRequest_success() throws Exception {
      ...
      mockMvc
          .perform(
              post("/groups/{groupId}/add-requests", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void createAddRequest_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(post("/groups/{groupId}/add-requests", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class GetAddRequestsTest {
    ...
    void getAddRequests_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/groups/{groupId}/add-requests", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void getAddRequests_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(get("/groups/{groupId}/add-requests", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class UpdateAddRequestTest {
    ...
    void updateAddRequest_accept_success() throws Exception {
      ...
      Authentication auth = createAuth(leader.getCustomerId());
      AddRequestDecisionDto request =
          new AddRequestDecisionDto(addRequest.getGroupAddRequestId(), true);

      // when & then
      mockMvc
          .perform(
              patch("/groups/{groupId}/add-requests", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void updateAddRequest_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              patch("/groups/{groupId}/add-requests", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
  ...
  class EntranceGroupTest {
    ...
    void entranceGroup_success() throws Exception {
      ...
      mockMvc
          .perform(
              post("/groups/{groupId}/entrance", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void entranceGroup_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/groups/{groupId}/entrance", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
  ...
  class ChangeGroupLeaderTest {
    ...
    void changeGroupLeader_success() throws Exception {
      ...
      Authentication auth = createAuth(leader.getCustomerId());
      GroupLeaderChangeRequestDto request =
          new GroupLeaderChangeRequestDto(newLeader.getCustomerId());

      // when & then
      mockMvc
          .perform(
              patch("/groups/{groupId}/group-leader", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void changeGroupLeader_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              patch("/groups/{groupId}/group-leader", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
  ...
  class ExpelMemberTest {
    ...
    void expelMember_success() throws Exception {
      ...
      mockMvc
          .perform(
              post("/groups/{groupId}/group-member", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth))
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk())
    ...
    void expelMember_unauthorized() throws Exception {
      ...
      mockMvc
          .perform(
              post("/groups/{groupId}/group-member", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isUnauthorized());
    }
  ...
  class LeaveGroupTest {
    ...
    void leaveGroup_success() throws Exception {
      ...
      mockMvc
          .perform(
              delete("/groups/{groupId}/group-member", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void leaveGroup_unauthorized() throws Exception {
      // when & then
      mockMvc
          .perform(delete("/groups/{groupId}/group-member", 1L))
          .andExpect(status().isUnauthorized());
    }
  }
  ...
  class DisbandGroupTest {
    ...
    void disbandGroup_success() throws Exception {
      ...
      mockMvc
          .perform(
              delete("/groups/{groupId}", group.getGroupId())
                  .with(SecurityMockMvcRequestPostProcessors.authentication(auth)))
          .andExpect(status().isOk())
    ...
    void disbandGroup_unauthorized() throws Exception {
      // when & then
      mockMvc.perform(delete("/groups/{groupId}", 1L)).andExpect(status().isUnauthorized());
    }
  }
}
