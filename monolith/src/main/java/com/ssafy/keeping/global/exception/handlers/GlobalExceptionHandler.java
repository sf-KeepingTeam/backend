...
public class GlobalExceptionHandler {
  ...
  /**
   * 0. JSON 파싱 실패 (@RequestBody 변환 단계에서 터짐) - 잘못된 JSON / 필드 타입 불일치 / enum 매핑 실패 등 - 기존에는 Exception
   * fallback에 걸려 500으로 나갔음
   */
  ...
  public ResponseEntity<ApiResponse<Void>> handleHttpMediaTypeNotSupported(
    ...
    return ResponseEntity.status(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
        .body(
            ApiResponse.error(
                "지원하지 않는 Content-Type 입니다. application/json으로 요청하세요.",
  ...
  public ResponseEntity<ApiResponse<Void>> handleTypeMismatch(
    ...
    return ResponseEntity.badRequest().body(ApiResponse.error(msg, HttpStatus.BAD_REQUEST.value()));
  }
  ...
  public ResponseEntity<ApiResponse<?>> handleDataIntegrity(
    ...
    log.error(
        "DataIntegrityViolationException at {}: {}", request.getRequestURI(), e.getMessage(), e);
  ...
  private boolean isUniqueViolation(Throwable t) {
    while (t != null) {
      if (t instanceof org.hibernate.exception.ConstraintViolationException cve) {
        log.error(
            "ConstraintViolationException - SQL: {}, SQLState: {}, ErrorCode: {}",
            cve.getSQL(),
            cve.getSQLState(),
            cve.getErrorCode());
