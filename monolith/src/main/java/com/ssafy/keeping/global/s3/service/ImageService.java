...
public class ImageService {
  ...
  public String updateProfileImage(String oldImgUrl, MultipartFile newImage) {
    ...
    try {
      putObjectRequest =
          new PutObjectRequest(bucketName, fileName, newImage.getInputStream(), metadata);
    } catch (IOException e) {
  ...
  public String uploadImage(MultipartFile image, String kindOfImage) {
    ...
    if (image == null || image.isEmpty()) {
      ...
      String basicImageFileName =
          "store".equals(kindOfImage.toLowerCase()) ? "storeBasicImage.jpg" : "menuBasicImage.jpg";

      return String.format(
          "https://%s.s3.%s.amazonaws.com/%s",
          bucketName, amazonS3.getRegionName(), basicImageFileName);
    }
  ...
  public boolean isS3Url(String url) {
    ...
    return url.contains("amazonaws.com") && url.contains("s3") && url.contains(bucketName);
  }
  ...
  private String getPublicUrl(String fileName) {
    return String.format(
        "https://%s.s3.%s.amazonaws.com/%s", bucketName, amazonS3.getRegionName(), fileName);
  }
}
