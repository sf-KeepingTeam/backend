 ...
 *
 * <p>Phase A (CancelReclaimService, @Transactional): 로컬 포인트 회수 + cancel TX 저장 Phase B (이 클래스, 트랜잭션
 * 밖): 토스 환불 호출 + 멱등키 재시도
 */
@Service
...
public class CancelService {
  ...
  /** 결제 취소 처리 — Saga (비-트랜잭션). Phase A: 로컬 포인트 회수 (별도 빈, TX) Phase B: 토스 환불 (TX 밖, 멱등키 재시도) */
  public CancelResponseDto cancelPayment(Long customerId, CancelRequestDto requestDto) {
    ...
    TossCancelRequest tossCancelRequest =
        TossCancelRequest.builder()
            ...
            .cancelAmount(
                requestDto.getCancelAmount() != null
    ...
    for (int attempt = 1; attempt <= MAX_TOSS_RETRY; attempt++) {
      try {
        TossCancelResponse tossResponse =
            tossPaymentClient.cancelPayment(reclaimResult.getPaymentKey(), tossCancelRequest);
        ...
        if (tossResponse.isSuccess() || isAlreadyDone(tossResponse)) {
          ...
          log.info(
              "[취소-PhaseB] 토스 환불 성공 - paymentKey: {}, attempt: {}, code: {}",
        ...
        if (isPermanentFailure(tossResponse)) {
          ...
          log.error(
              "[취소-PhaseB] 토스 환불 영구 실패 - code: {}, message: {}, paymentKey: {}",
              tossResponse.getCode(),
        ...
        // 일시 실패 → 재시도
        log.warn(
            "[취소-PhaseB] 토스 환불 일시 실패 (attempt {}/{}) - code: {}, message: {}",
            attempt,
            MAX_TOSS_RETRY,
            tossResponse.getCode(),
            tossResponse.getMessage());
        ...
        lastException = e;
        log.warn(
            "[취소-PhaseB] 토스 환불 예외 (attempt {}/{}) - {}", attempt, MAX_TOSS_RETRY, e.getMessage());
      }
    ...
    if (finalStatus == RefundStatus.REFUND_PERMANENT_FAILED) {
      log.error(
          "[취소-PhaseB] 포인트 회수 완료됐으나 토스 환불 영구 실패 - paymentKey: {}, 수동 확인 필요",
          reclaimResult.getPaymentKey());
    ...
    log.error(
        "[취소-PhaseB] 토스 환불 {}회 재시도 실패 - paymentKey: {}, REFUND_PENDING 유지. "
  ...
  /** "이미 취소됨" 판정 — 돈이 이미 환불된 상태이므로 성공(REFUND_DONE)으로 처리해야 한다. 이를 영구실패로 처리하면 운영자 재환불 → 이중환불 위험. */
  ...
  /**
   * 토스 에러 코드 기반 영구 실패 판정. 영구 실패 = 재시도해도 결과가 바뀌지 않는 에러 (4xx 계열). 주의: ALREADY_CANCELED는 여기가 아니라
   * isAlreadyDone()에서 성공으로 처리.
   */
