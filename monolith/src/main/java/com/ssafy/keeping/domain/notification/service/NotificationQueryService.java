...
public class NotificationQueryService {
  ...
  public void markAsReadForOwner(Long ownerId, Long notificationId) {
    ...
    Notification notification =
        notificationRepository
            .findByNotificationIdAndOwnerId(notificationId, ownerId)
            .orElseThrow(() -> new CustomException(ErrorCode.NOTIFICATION_NOT_FOUND));
  ...
  public Page<NotificationResponseDto> getNotificationListForCustomer(
    ...
    log.info(
        "고객 알림 목록 조회 완료 - 고객ID: {}, 총 알림 수: {}, 페이지 알림 수: {}",
        customerId,
   ...
   */
  @Transactional(readOnly = true)
  public Page<NotificationResponseDto> getNotificationListForOwner(
    ...
    // 알림 목록 조회
    Page<Notification> notifications =
        notificationRepository.findByOwnerIdOrderByCreatedAtDesc(ownerId, pageable);

    log.info(
        "점주 알림 목록 조회 완료 - 점주ID: {}, 총 알림 수: {}, 페이지 알림 수: {}",
        ownerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());
   ...
   */
  @Transactional(readOnly = true)
  public Page<NotificationResponseDto> getUnreadNotificationListForCustomer(
      Long customerId, Pageable pageable) {
    ...
    // 읽지 않은 알림 목록 조회
    Page<Notification> notifications =
        notificationRepository.findUnreadNotificationsByCustomerIdOrderByCreatedAtDesc(
            customerId, pageable);

    log.info(
        "고객 읽지 않은 알림 목록 조회 완료 - 고객ID: {}, 총 읽지 않은 알림 수: {}, 페이지 알림 수: {}",
        customerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());
   ...
   */
  @Transactional(readOnly = true)
  public Page<NotificationResponseDto> getUnreadNotificationListForOwner(
      Long ownerId, Pageable pageable) {
    ...
    // 읽지 않은 알림 목록 조회
    Page<Notification> notifications =
        notificationRepository.findUnreadNotificationsByOwnerIdOrderByCreatedAtDesc(
            ownerId, pageable);

    log.info(
        "점주 읽지 않은 알림 목록 조회 완료 - 점주ID: {}, 총 읽지 않은 알림 수: {}, 페이지 알림 수: {}",
        ownerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());
