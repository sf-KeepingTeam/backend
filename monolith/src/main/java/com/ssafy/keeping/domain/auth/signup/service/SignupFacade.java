...
public class SignupFacade {
  ...
  public IssuedAuthTokens signupCustomer(CustomerSignupRequest req) {
    ...
    return issueSuccessToken(
        result.userId(), result.role(), result.userDto()); // 커밋 이후에 토큰 발급/저장 수행
  }
  ...
  /** 토큰 발급 + 응답 DTO 생성 access + refresh 모두 발급 */
  private IssuedAuthTokens issueSuccessToken(long userId, UserRole role, RegisterResponse userDto) {
    ...
    SignupResponse body = new SignupResponse(userDto, AuthResponse.bearer(accessToken, role));
