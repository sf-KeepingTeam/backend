...
public interface SettlementTaskRepository extends JpaRepository<SettlementTask, Long> {
  ...
  /** 이전 주의 PENDING 상태 정산 작업 조회 (월요일 07:30 실행용) 지난 주 월요일 07:30부터 월요일 07:30까지의 PENDING 상태 작업 */
  @Query(
      "SELECT st FROM SettlementTask st WHERE st.status = 'PENDING' "
          + "AND st.createdAt >= :weekStart AND st.createdAt < :weekEnd ORDER BY st.createdAt ASC")
  List<SettlementTask> findPendingTasksFromPreviousWeek(
      @Param("weekStart") LocalDateTime weekStart, @Param("weekEnd") LocalDateTime weekEnd);
  ...
  /**
   * 특정 고객의 취소 가능한 거래 목록 조회 (페이지네이션) 조건: 1) PENDING 상태, 2) CHARGE 타입 wallet_store_lot, 3) 미사용 포인트
   * (총량=잔액)
   */
  @Query(
      "SELECT st FROM SettlementTask st "
  ...
  Page<SettlementTask> findCancelableTransactions(
