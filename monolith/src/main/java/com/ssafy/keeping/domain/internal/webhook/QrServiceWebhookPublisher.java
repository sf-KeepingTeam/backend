...
/** QR Service로 캐시 갱신 Webhook 전송 Fire-and-forget 패턴: 실패해도 예외 무시 */
@Slf4j
@Service
...
public class QrServiceWebhookPublisher {
  ...
  /** Store 생성/수정 시 캐시 갱신 Push 실패 시 최대 3회 재시도 (1s → 2s → 4s) */
  ...
  public void recoverStoreUpdate(Exception e, Store store) {
    log.warn(
        "Store 캐시 Push 최종 실패 (3회 재시도 후): storeId={}, error={}", store.getStoreId(), e.getMessage());
  }

  /** Store 삭제 시 캐시 삭제 Push 실패 시 최대 3회 재시도 (1s → 2s → 4s) */
  @Async("webhookExecutor")
  @Retryable(
      retryFor = {RestClientException.class, ResourceAccessException.class},
      maxAttempts = 4,
  ...
  public void recoverStoreDelete(Exception e, Long storeId) {
    log.warn("Store 캐시 삭제 Push 최종 실패 (3회 재시도 후): storeId={}, error={}", storeId, e.getMessage());
  }

  /** Menu 생성/수정 시 캐시 갱신 Push 실패 시 최대 3회 재시도 (1s → 2s → 4s) */
  @Async("webhookExecutor")
  @Retryable(
      retryFor = {RestClientException.class, ResourceAccessException.class},
      maxAttempts = 4,
  ...
  public void recoverMenuUpdate(Exception e, Menu menu) {
    log.warn(
        "Menu 캐시 Push 최종 실패 (3회 재시도 후): menuId={}, error={}", menu.getMenuId(), e.getMessage());
  }

  /** Menu 삭제 시 캐시 삭제 Push 실패 시 최대 3회 재시도 (1s → 2s → 4s) */
  @Async("webhookExecutor")
  @Retryable(
      retryFor = {RestClientException.class, ResourceAccessException.class},
      maxAttempts = 4,
  ...
  public void recoverMenuDelete(Exception e, Long menuId) {
    log.warn("Menu 캐시 삭제 Push 최종 실패 (3회 재시도 후): menuId={}, error={}", menuId, e.getMessage());
  }
