...
/**
 * 전액 결제 취소 서비스 (멱등 패턴: initiate 와 동일 흐름) - 멱등 스코프: (actorType=MERCHANT, actorId=ownerId,
 * path=/api/stores/{storeId}/transactions/{txId}/refund, key=Idempotency-Key) - 상태 흐름: DONE → 저장된
 * 응답 재생(200 OK) IN_PROGRESS(선점 중) → 202 Accepted (+ Retry-After) 신규 → 본 처리 수행 → DONE 기록 후 201
 * Created
 */
@Slf4j
@Service
...
public class PaymentRefundService {
   ...
   */
  @Transactional
  public IdempotentResult<RefundResponse> fullCancel(
      Long storeId, Long transactionId, String idempotencyKeyHeader, Long ownerId) {
    ...
    if (transactionRepository.existsByRefTxIdAndType(
        original.getTransactionId(), TransactionType.CANCEL_USE)) {
      Transaction cancelTx =
          transactionRepository
              .findCancelByRef(original.getTransactionId(), TransactionType.CANCEL_USE)
    ...
    String path =
        "/api/stores/" + storeId + "/transactions/" + transactionId + "/refund"; // 스코프 정규화
    IdemBegin begin =
        idempotencyService.beginOrLoad(
            IdemActorType.MERCHANT, ownerId, "POST", path, keyUuid, EMPTY_BODY_HASH);
    ...
    WalletStoreBalance balance =
        walletStoreBalanceRepository
            .findByWalletAndStoreForUpdate(wallet, store)
            .orElseGet(
                () -> {
                  try {
                    return walletStoreBalanceRepository.save(
                    ...
                    return walletStoreBalanceRepository
                        .findByWalletAndStoreForUpdate(wallet, store)
                        .orElseThrow(() -> e);
                  }
                });
    ...
    try {
      String notificationContent =
          String.format("%s에서 %,d포인트 사용이 취소되었습니다.", store.getStoreName(), original.getAmount());
      ...
      log.info(
          "결제 수락 알림 전송 완료 - 손님ID: {}, 결제 금액: {}, 사용 가게 ID: {}",
          original.getCustomer().getCustomerId(),
          original.getAmount(),
          store.getStoreId());
    } catch (Exception e) {
      log.warn(
          "결제 수락 알림 전송 실패 - 손님ID: {}, 결제 금액: {}, 사용 가게 ID: {}",
          original.getCustomer().getCustomerId(),
          original.getAmount(),
          store.getStoreId());
    }
