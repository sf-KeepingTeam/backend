...
public class StoreStatisticsService {
  ...
  public StoreOverallStatisticsResponseDto getOverallStatistics(Long storeId, Long ownerId) {
    ...
    Long totalPaymentAmount =
        transactionRepository.getTotalPaymentAmountByStore(storeId); // 총 결제 금액 - 취소는 조회 안하게끔함
    Long totalChargePoints =
        transactionRepository.getTotalChargePointsByStore(storeId); // 총 충전 포인트 (보너스 포함)
    Long totalPointsUsed = transactionRepository.getTotalPointsUsedByStore(storeId); // point 총 사용량
    Long totalTransactionCount =
        transactionRepository.getTotalTransactionCountByStore(
            storeId); // 총 포인트 거래 수 (충전, 취소, 사용, 회수 포함)
  ...
  /** 가게 일별 통계 조회 */
  public DailyStatisticsResponseDto getDailyStatistics(
      Long storeId, Long ownerId, StatisticsRequestDto requestDto) {
    log.info("일별 통계 조회 시작 - 가게ID: {}, 점주ID: {}, 날짜: {}", storeId, ownerId, requestDto.getDate());
    ...
    Long dailyTotalChargePoints =
        transactionRepository.getDailyTotalChargePointsByStore(storeId, date);
    ...
    Long dailyTransactionCount =
        transactionRepository.getDailyTransactionCountByStore(storeId, date); // 모든 거래 타입 포함

    log.info(
        "일별 통계 조회 완료 - 가게: {}, 날짜: {}, 결제: {}원, 사용: {}포인트",
        store.getStoreName(),
        date,
        dailyPaymentAmount,
        dailyPointsUsed);
  ...
  /** 가게 기간별 통계 조회 */
  public PeriodStatisticsResponseDto getPeriodStatistics(
      Long storeId, Long ownerId, StatisticsRequestDto requestDto) {
    log.info(
        "기간별 통계 조회 시작 - 가게ID: {}, 점주ID: {}, 기간: {} ~ {}",
        storeId,
        ownerId,
    ...
    // 3. 통계 데이터 조회
    Long periodPaymentAmount =
        transactionRepository.getPeriodPaymentAmountByStore(storeId, startDate, endDate);
    Long periodTotalChargePoints =
        transactionRepository.getPeriodTotalChargePointsByStore(storeId, startDate, endDate);
    Long periodPointsUsed =
        transactionRepository.getPeriodPointsUsedByStore(storeId, startDate, endDate);
    Long periodChargeCount =
        transactionRepository.getPeriodChargeCountByStore(storeId, startDate, endDate);
    Long periodUseCount =
        transactionRepository.getPeriodUseCountByStore(storeId, startDate, endDate);
    Long periodTransactionCount =
        transactionRepository.getPeriodTransactionCountByStore(
            storeId, startDate, endDate); // 모든 거래 타입 포함
    ...
    log.info(
        "기간별 통계 조회 완료 - 가게: {}, 기간: {}일, 결제: {}원, 사용: {}포인트",
        store.getStoreName(),
        daysBetween,
        periodPaymentAmount,
        periodPointsUsed);
  ...
  /** 가게 월별 통계 조회 */
  public MonthlyStatisticsResponseDto getMonthlyStatistics(
      Long storeId, Long ownerId, StatisticsRequestDto requestDto) {
    ...
    Long monthlyPaymentAmount =
        transactionRepository.getMonthlyPaymentAmountByStore(storeId, year, month);
    Long monthlyTotalChargePoints =
        transactionRepository.getMonthlyTotalChargePointsByStore(storeId, year, month);
    Long monthlyPointsUsed =
        transactionRepository.getMonthlyPointsUsedByStore(storeId, year, month);
    Long monthlyChargeCount =
        transactionRepository.getMonthlyChargeCountByStore(storeId, year, month);
    Long monthlyUseCount = transactionRepository.getMonthlyUseCountByStore(storeId, year, month);
    Long monthlyTransactionCount =
        transactionRepository.getMonthlyTransactionCountByStore(storeId, year, month);
  ...
  private Store validateStoreOwnership(Long storeId, Long ownerId) {
    ...
    if (!store.getOwner().getOwnerId().equals(ownerId)) {
      log.warn("가게 소유권 없음 - 요청 점주ID: {}, 실제 점주ID: {}", ownerId, store.getOwner().getOwnerId());
