 ...
 *
 * <p>loadtest.backdoor.enabled=true 일 때만 활성화됩니다. Kakao OAuth 없이 userId + role 만으로 AccessToken +
 * RefreshToken을 즉시 발급합니다.
 *
 * <p>사용 예시: POST /auth/dev-login { "userId": 1, "role": "CUSTOMER" }
 *
 * <p>응답: - body: { accessToken, tokenType, expiresIn, role, userId } - Set-Cookie:
 * REFRESH_TOKEN=<uuid>; HttpOnly; Path=/auth
 */
@RestController
@RequestMapping("/auth")
    ...
    return ResponseEntity.ok()
        ...
        .body(
            new DevLoginResponse(
                accessToken, "Bearer", accessTokenService.accessTtlSeconds(), role.name(), userId));
  }

  public record DevLoginRequest(Long userId, String role // "CUSTOMER" 또는 "OWNER"
      ) {}

  public record DevLoginResponse(
      String accessToken, String tokenType, long expiresIn, String role, Long userId) {}
}
