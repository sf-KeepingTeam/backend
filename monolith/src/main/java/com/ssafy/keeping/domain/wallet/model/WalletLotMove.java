...
public class WalletLotMove {
  ...
  @ManyToOne(fetch = FetchType.LAZY, optional = false)
  @JoinColumn(name = "lot_id", nullable = false, foreignKey = @ForeignKey(name = "fk_moves_lot"))
  ...
  public static WalletLotMove of(Transaction tx, WalletStoreLot lot, long delta) {
    ...
    return WalletLotMove.builder().transaction(tx).lot(lot).delta(delta).build();
  }
}
