...
/**
 * 정산 스케줄러 (포인트 기반) - 선결제 시 점주에게 즉시 포인트 적립됨 - 이 스케줄러는 정산 상태 관리 및 기록 보관 용도로만 사용
 *
 * <p>간소화: 정산 시스템 비활성화
 */
// @Service
...
public class SettlementScheduler {
  ...
  /** 매주 월요일 오전 07:30에 이전 주 PENDING 작업들을 LOCKED 상태로 변경 취소 불가 상태로 전환 (취소 기간 종료) */
  ...
  public void lockPreviousWeekTasks() {
    ...
    try {
      ...
      LocalDateTime thisWeekMondayBilling =
      ...
      List<SettlementTask> pendingTasks =
          settlementTaskRepository.findPendingTasksFromPreviousWeek(
              lastWeekMondayBilling, thisWeekMondayBilling);
  ...
  /** 매주 화요일 오전 01:00에 LOCKED 상태 작업들을 COMPLETED로 변경 (포인트는 이미 선결제 시 즉시 적립되었으므로, 상태만 변경) */
  ...
  public void processLockedSettlements() {
    ...
    try {
      ...
      // 가게별로 그룹화하여 로깅
      Map<Store, List<SettlementTask>> tasksByStore =
          lockedTasks.stream()
      ...
      for (Map.Entry<Store, List<SettlementTask>> entry : tasksByStore.entrySet()) {
        ...
        Long totalAmount =
            tasks.stream().map(SettlementTask::getActualPaymentAmount).reduce(0L, Long::sum);

        log.info(
            "가게별 정산 완료 처리 - 가게: {}, 작업 수: {}, 총 금액: {}",
            store.getStoreName(),
            tasks.size(),
            totalAmount);
