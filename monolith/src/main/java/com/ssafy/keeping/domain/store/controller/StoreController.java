...
public class StoreController {
  ...
  public ResponseEntity<ApiResponse<List<StorePublicDto>>> getAllStore() {
    return ResponseEntity.ok(
        ApiResponse.success("전체 매장이 조회되었습니다", HttpStatus.OK.value(), storeService.getAllStore()));
  }

  @GetMapping("/{storeId}")
  public ResponseEntity<ApiResponse<StorePublicDto>> getStore(@PathVariable Long storeId) {
    return ResponseEntity.ok(
        ApiResponse.success(
            "해당 store id로 매장이 조회되었습니다.",
            HttpStatus.OK.value(),
  ...
  public ResponseEntity<ApiResponse<List<StorePublicDto>>> getAllStoreByCategory(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            "해당 category로 매장이 조회되었습니다.",
            HttpStatus.OK.value(),
            storeService.getAllStoreByCategory(category)));
  }

  @GetMapping(params = "name")
  public ResponseEntity<ApiResponse<List<StorePublicDto>>> getStore(@RequestParam String name) {
    return ResponseEntity.ok(
        ApiResponse.success(
            "store name으로 매장이 조회되었습니다.",
            HttpStatus.OK.value(),
