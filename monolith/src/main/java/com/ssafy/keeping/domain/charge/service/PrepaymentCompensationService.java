...
/**
 * 충전 보상 전담 빈.
 * REQUIRES_NEW로 독립 커밋 — 외부 트랜잭션 롤백에 먹히지 않는다.
 */
@Service
...
public class PrepaymentCompensationService {
    ...
    /**
     * 보상: 토스 cancel + 예약 FAILED 마킹.
     * 독립 트랜잭션(REQUIRES_NEW)으로 외부 롤백에 영향받지 않는다.
     * 가드: 예약이 이미 COMPLETED면 FAILED로 덮어쓰지 않는다.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void compensate(Long reservationId, String paymentKey, String reason) {
        log.warn("[충전-보상] 시작 - reservationId: {}, paymentKey: {}, reason: {}",
                reservationId, paymentKey, reason);
        ...
        try {
            TossCancelResponse cancelResponse = tossPaymentClient.cancelPayment(paymentKey, cancelRequest);
            if (cancelResponse.isSuccess()) {
                log.info("[충전-보상] 토스 결제 취소 성공 - paymentKey: {}", paymentKey);
            } else {
                log.error("[충전-보상] 토스 결제 취소 실패 - paymentKey: {}, code: {}, message: {}",
                        paymentKey, cancelResponse.getCode(), cancelResponse.getMessage());
            }
        } catch (Exception e) {
            log.error("[충전-보상] 토스 결제 취소 예외 - paymentKey: {}, error: {}",
    ...
    /**
     * 토스 confirm 실패 시 예약만 FAILED 마킹 (독립 tx).
     * 가드: 이미 COMPLETED면 FAILED로 덮어쓰지 않는다.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
