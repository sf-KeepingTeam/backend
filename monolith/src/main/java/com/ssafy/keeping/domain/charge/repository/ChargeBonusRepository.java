...
public interface ChargeBonusRepository extends JpaRepository<ChargeBonus, Long> {
  ...
  @Query(
      "SELECT COUNT(cb) > 0 FROM ChargeBonus cb WHERE cb.store = :store AND cb.chargeAmount = :chargeAmount AND cb.chargeBonusId != :excludeId")
  boolean existsByStoreAndChargeAmountExcludingId(
