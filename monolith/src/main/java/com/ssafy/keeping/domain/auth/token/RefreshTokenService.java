...
public class RefreshTokenService {
  ...
  /**
   * 한 사용자당 refresh 토큰 1개만 유지 - 새로 발급하면 기존 토큰은 무효화
   *
   * @param userId
   * @param role
   * @return
  ...
  public IssuedRefreshToken issueSingleSession(long userId, UserRole role) {
    ...
    RefreshTokenPayload payload =
        new RefreshTokenPayload(userId, role, Instant.now().getEpochSecond());
  ...
  /**
   * refresh 로테이션 (쿠키 기반 재발급에서 사용) 1) refreshToken -> payload 검증 2) ACTIVE(role:id)에 저장된 "현재 토큰"인지
   * 확인 3) 새 토큰 발급 + TOKEN/ACTIVE 갱신 + 기존 토큰 삭제
   */
  ...
  /** 로그아웃용: TOKEN/ACTIVE 둘 다 제거 - 로그아웃은 여러 번 호출되어도 성공(idempotent)하는 게 UX가 좋음 */
