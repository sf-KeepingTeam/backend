...
/** 결제 상태 확인 서비스 - 멱등성 키를 통해 결제 존재 여부 확인 */
@Slf4j
@Service
...
public class InternalPaymentService {
  ...
  /** 멱등성 키로 결제 존재 여부 확인 - 완료된 결제가 있으면 트랜잭션 정보 반환 - 없으면 exists=false 반환 */
  ...
  public PaymentCheckResponse checkPayment(String idempotencyKeyStr) {
    ...
    try {
      ...
      return PaymentCheckResponse.builder().exists(false).build();
    }

    return idempotencyKeyRepository
        .findByKeyUuid(keyUuid)
        .filter(key -> key.getStatus() == IdemStatus.DONE)
        .map(this::buildResponseFromIdempotencyKey)
        .orElse(PaymentCheckResponse.builder().exists(false).build());
  }
  ...
  private PaymentCheckResponse buildResponseFromIdempotencyKey(IdempotencyKey key) {
    try {
      ...
      FundsResponse snapshot =
          canonicalObjectMapper.treeToValue(key.getResponseJson(), FundsResponse.class);
