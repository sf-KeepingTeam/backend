...
public interface FcmTokenRepository extends JpaRepository<FcmToken, Long> {
  ...
  Optional<FcmToken> findByCustomerIdAndToken(
      @Param("customerId") Long customerId, @Param("token") String token);

  /** 점주의 특정 토큰 조회 */
  @Query("SELECT f FROM FcmToken f WHERE f.owner.ownerId = :ownerId AND f.token = :token")
  Optional<FcmToken> findByOwnerIdAndToken(
      @Param("ownerId") Long ownerId, @Param("token") String token);
