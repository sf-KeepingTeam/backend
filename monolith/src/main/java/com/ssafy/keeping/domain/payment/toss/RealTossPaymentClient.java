...
/**
 * 실제 토스페이먼츠 API를 호출하는 클라이언트. payment.toss.mode=real 일 때 활성화된다. 인증: Basic base64(secretKey + ":")
 */
@Slf4j
@Component
...
public class RealTossPaymentClient implements TossPaymentClient {
  ...
  public RealTossPaymentClient(TossPaymentConfig config, RestClient.Builder builder) {
    ...
    this.restClient = builder.baseUrl(config.getBaseUrl()).requestFactory(factory).build();
  ...
  public TossPaymentConfirmResponse confirmPayment(TossPaymentConfirmRequest request) {
    log.info(
        "[TossReal] 결제 승인 호출 - orderId: {}, amount: {}", request.getOrderId(), request.getAmount());
    ...
    if (!response.isSuccess()) {
      log.warn(
          "[TossReal] 결제 승인 실패 - code: {}, message: {}", response.getCode(), response.getMessage());
    }
    return response;
  }
  ...
  public TossCancelResponse cancelPayment(String paymentKey, TossCancelRequest request) {
    log.info(
        "[TossReal] 결제 취소 호출 - paymentKey: {}, cancelAmount: {}",
        paymentKey,
    ...
    if (!response.isSuccess()) {
      log.warn(
          "[TossReal] 결제 취소 실패 - code: {}, message: {}", response.getCode(), response.getMessage());
    }
    return response;
  }
}
