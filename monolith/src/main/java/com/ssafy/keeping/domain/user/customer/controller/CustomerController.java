...
public class CustomerController {
  ...
  /** 프로필 이미지 수정 */
  @PostMapping(
      value = "/{customerId}/profile-image/upload",
      consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
  public ResponseEntity<ApiResponse<ProfileUploadResponse>> uploadProfileImage(
      @PathVariable Long customerId, @RequestParam("file") MultipartFile file) {
  ...
  public ResponseEntity<ApiResponse<CustomerProfileResponse>> updateMyProfile(
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(ApiResponse.success("프로필 수정 성공", HttpStatus.OK.value(), response));
  }

  /** 내가 속한 그룹 조회 */
  @GetMapping("/me/groups")
  public ResponseEntity<ApiResponse<MyGroupsResponse>> myGroups(
      @AuthenticationPrincipal UserPrincipal principal) {
    Long customerId = principal.id();
    List<Long> ids = groupMemberRepository.findGroupIdsByCustomerId(customerId);
    MyGroupsResponse data = MyGroupsResponse.of(ids);
    return ResponseEntity.status(HttpStatus.OK)
        .body(ApiResponse.success("내가 속한 그룹을 조회하였습니다.", HttpStatus.CREATED.value(), data));
  }
}
