...
/**
 * 토스페이먼츠 Stub 클라이언트. 실제 토스 API를 호출하지 않고 항상 성공 응답을 반환한다. payment.toss.mode 가 없거나 stub 일 때 활성화된다
 * (기본값).
 */
@Slf4j
@Component
...
public class StubTossPaymentClient implements TossPaymentClient {
  ...
  public TossPaymentConfirmResponse confirmPayment(TossPaymentConfirmRequest request) {
    log.info(
        "[TossStub] 결제 승인 stub 처리 - orderId: {}, amount: {}",
        request.getOrderId(),
        request.getAmount());
    ...
    response.setPaymentKey(
        request.getPaymentKey() != null ? request.getPaymentKey() : "stub_pk_" + UUID.randomUUID());
  ...
  public TossCancelResponse cancelPayment(String paymentKey, TossCancelRequest request) {
    log.info(
        "[TossStub] 결제 취소 stub 처리 - paymentKey: {}, cancelAmount: {}",
        paymentKey,
