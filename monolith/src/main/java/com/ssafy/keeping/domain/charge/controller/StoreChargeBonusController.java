...
public class StoreChargeBonusController {
  ...
  /**
   * 고객용 충전 보너스 목록 조회 만약 10000원을 충전하면 11000포인트를 줄거다! 하는거임 (그 목록을 프론트에게 보내주기 위함) 가게 페이지 보여줄 때 필요할 듯
   */
  @GetMapping
  public ResponseEntity<ApiResponse<List<PublicChargeBonusResponseDto>>> getPublicChargeBonusList(
    ...
    List<PublicChargeBonusResponseDto> responseDtoList =
        bonusList.stream().map(PublicChargeBonusResponseDto::from).collect(Collectors.toList());
