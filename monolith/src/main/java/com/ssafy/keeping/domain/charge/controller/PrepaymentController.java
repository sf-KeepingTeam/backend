...
public class PrepaymentController {
  ...
  /**
   * [1단계] 선결제 예약 (보안 강화) 서버에서 금액을 먼저 확정하여 금액 변조 방지
   *
   * @param storeId 가게 ID
  ...
  public ResponseEntity<ApiResponse<PrepaymentReserveResponse>> reservePrepayment(
    ...
    PrepaymentReserveResponse response =
        prepaymentService.reservePayment(storeId, customerId, request);

    //        log.info("[예약] 응답 전송 - orderId: {}, 만료: {}", response.getOrderId(),
  ...
  /** [2단계] 프론트엔드 - 토스 결제창 호출 성공하면 3단계로 호출 */

  /**
   * [3단계] 선결제 승인 (보안 강화) 예약된 금액과 비교하여 변조 방지 및 멱등성 보장
   *
   * @param storeId 가게 ID
   * @param principal 인증 정보
  ...
  public ResponseEntity<ApiResponse<PrepaymentResponseDto>> confirmPrepayment(
    ...
    IdempotentResult<PrepaymentResponseDto> result =
        prepaymentService.confirmPayment(storeId, customerId, request);
