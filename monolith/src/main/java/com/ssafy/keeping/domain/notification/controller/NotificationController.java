...
public class NotificationController {
   ...
   * @return SseEmitter 로그인 시에 이걸 불러서 구독하도록 할 것임
   */
  @GetMapping(
      value = "/subscribe/customer/{customerId}",
  ...
  public SseEmitter subscribeCustomer(
    ...
    log.info(
        "고객 SSE 구독 요청 - 고객ID: {}, Last-Event-ID: {}, Authorization: {}",
        customerId,
        lastEventId,
        authorizationHeader != null ? "있음" : "없음");
  ...
  public SseEmitter subscribeOwner(
    ...
    log.info(
        "점주 SSE 구독 요청 - 점주ID: {}, Last-Event-ID: {}, Authorization: {}",
        ownerId,
        lastEventId,
        authorizationHeader != null ? "있음" : "없음");
  ...
  public ResponseEntity<ApiResponse<Page<NotificationResponseDto>>> getNotificationListForCustomer(
    ...
    Page<NotificationResponseDto> notifications =
        notificationQueryService.getNotificationListForCustomer(customerId, pageable);

    log.info(
        "고객 알림 목록 조회 성공 - 고객ID: {}, 총 개수: {}, 현재 페이지 개수: {}",
        customerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());

    return ResponseEntity.ok(
        ApiResponse.success("고객 알림 목록 조회 완료", HttpStatus.OK.value(), notifications));
  }
  ...
  public ResponseEntity<ApiResponse<Page<NotificationResponseDto>>> getNotificationListForOwner(
    ...
    Pageable pageable = PageRequest.of(page, size);
    Page<NotificationResponseDto> notifications =
        notificationQueryService.getNotificationListForOwner(ownerId, pageable);

    log.info(
        "점주 알림 목록 조회 성공 - 점주ID: {}, 총 개수: {}, 현재 페이지 개수: {}",
        ownerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());

    return ResponseEntity.ok(
        ApiResponse.success("점주 알림 목록 조회 완료", HttpStatus.OK.value(), notifications));
  }
  ...
  public ResponseEntity<ApiResponse<Long>> getUnreadCountForCustomer(
    ...
    return ResponseEntity.ok(
        ApiResponse.success("읽지 않은 알림 개수 조회 완료", HttpStatus.OK.value(), unreadCount));
  }
  ...
  public ResponseEntity<ApiResponse<Page<NotificationResponseDto>>>
      getUnreadNotificationListForCustomer(
          @PathVariable @Positive(message = "고객 ID는 양수여야 합니다.") Long customerId,
          @RequestParam(defaultValue = "0") @PositiveOrZero(message = "페이지 번호는 0 이상이어야 합니다.")
              int page,
          @RequestParam(defaultValue = "20") @Positive(message = "페이지 크기는 양수여야 합니다.") int size) {
    ...
    Pageable pageable = PageRequest.of(page, size);
    Page<NotificationResponseDto> notifications =
        notificationQueryService.getUnreadNotificationListForCustomer(customerId, pageable);

    log.info(
        "고객 읽지 않은 알림 목록 조회 성공 - 고객ID: {}, 총 개수: {}, 현재 페이지 개수: {}",
        customerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());

    return ResponseEntity.ok(
        ApiResponse.success("고객 읽지 않은 알림 목록 조회 완료", HttpStatus.OK.value(), notifications));
  }
  ...
  public ResponseEntity<ApiResponse<Page<NotificationResponseDto>>>
      getUnreadNotificationListForOwner(
          @PathVariable @Positive(message = "점주 ID는 양수여야 합니다.") Long ownerId,
          @RequestParam(defaultValue = "0") @PositiveOrZero(message = "페이지 번호는 0 이상이어야 합니다.")
              int page,
          @RequestParam(defaultValue = "20") @Positive(message = "페이지 크기는 양수여야 합니다.") int size) {
    ...
    Pageable pageable = PageRequest.of(page, size);
    Page<NotificationResponseDto> notifications =
        notificationQueryService.getUnreadNotificationListForOwner(ownerId, pageable);

    log.info(
        "점주 읽지 않은 알림 목록 조회 성공 - 점주ID: {}, 총 개수: {}, 현재 페이지 개수: {}",
        ownerId,
        notifications.getTotalElements(),
        notifications.getNumberOfElements());

    return ResponseEntity.ok(
        ApiResponse.success("점주 읽지 않은 알림 목록 조회 완료", HttpStatus.OK.value(), notifications));
  }
