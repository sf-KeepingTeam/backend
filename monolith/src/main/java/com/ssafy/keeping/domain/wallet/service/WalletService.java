...
public class WalletService { // 충돌나는 것을 방지해 HS를 붙였으나 추후 합치겠습니다.
            ...
            () -> {
              try {
                return walletRepository.save(
                    Wallet.builder().walletType(WalletType.INDIVIDUAL).customer(customer).build());
  ...
  public WalletResponseDto createGroupWallet(Group group) {
    ...
    Wallet saved =
        walletRepository.save(Wallet.builder().walletType(WalletType.GROUP).group(group).build());
  ...
  public WalletResponseDto getGroupWallet(Group group) {
    ...
    List<WalletStoreBalanceResponseDto> groupStoreBalanceDtoList =
        Optional.ofNullable(groupWallet.getWalletStoreBalances())
            ...
            .map(
                b ->
                    new WalletStoreBalanceResponseDto(
                        b.getBalanceId(), b.getBalance(), b.getUpdatedAt()))
            .toList();
  ...
  public IdempotentResult<PointShareResponseDto> sharePoints(
      ...
      @Valid PointShareRequestDto req) {
  ...
  // ===== 실제 처리 본문  =====
  @Transactional
  protected PointShareResponseDto doSharePoints(
      Long groupId, Long userId, Long storeId, @Valid PointShareRequestDto req) {
    // 1) 입력·기본 엔티티 조회
    ...
    if (individual.getWalletType() != WalletType.INDIVIDUAL
        || group.getWalletType() != WalletType.GROUP)
    ...
    WalletStoreBalance groupBal =
        balanceRepository
            .lockByWalletIdAndStoreId(group.getWalletId(), storeId)
            .orElseGet(
                () ->
    ...
    List<WalletStoreLot> lots =
        lotRepository.lockAllByWalletIdAndStoreIdOrderByAcquiredAt(
            individual.getWalletId(), storeId);
    ...
    String message =
        String.format(
            ...
            actor.getName(), group.getGroup().getGroupName(), store.getStoreName(), shareAmount);
    ...
    return new PointShareResponseDto(
        txOut.getTransactionId(),
        txIn.getTransactionId(),
  ...
  // 실제 회수 처리
  @Transactional
  protected PointShareResponseDto doReclaimPoints(
      Long groupId, Long userId, Long storeId, @Valid PointShareRequestDto req) {
    ...
    WalletStoreBalance groupBal =
        balanceRepository
            .lockByWalletIdAndStoreId(group.getWalletId(), storeId)
            .orElseThrow(
                () -> new CustomException(ErrorCode.BEFORE_GROUP_CHARGE)); // 그룹에 해당 매장 잔액이 있어야 함
  ...
  // ===== Helpers =====
  private String canonicalizeShareBody(
      Long groupId, Long userId, Long storeId, PointShareRequestDto req) {
    // 키 순서 고정 직렬화
  ...
  // === canonical helpers ===
  private String canonicalizeReclaimBody(
      Long groupId, Long userId, Long storeId, PointShareRequestDto req) {
    ObjectNode n = canonicalObjectMapper.createObjectNode();
    n.put("groupId", groupId);
    n.put("userId", userId);
  ...
  public AvailablePointResponseDto getReclaimablePoints(
      Long walletId, Long storeId, Long customerId) {
  ...
  public long getMemberSharedBalanceByStore(Long groupWalletId, Long storeId, Long customerId) {
    List<WalletStoreLot> lots =
        lotRepository.findReclaimableByStore(
  ...
  public long getMemberSharedBalance(Long groupId, Long customerId) { // 시그니처 변경
    ...
    List<WalletStoreLot> lots =
        lotRepository.findActiveByWalletIdAndContributorCustomerId(
            groupWallet.getWalletId(), customerId);
    return lots.stream().mapToLong(WalletStoreLot::getAmountRemaining).sum();
  }
  ...
  public long getTotalIndividualBalance(Long customerId) {
    return balanceRepository
        .sumBalanceByCustomerIdAndType(customerId, WalletType.INDIVIDUAL)
        .orElse(0L);
  }
  ...
  public PersonalWalletBalanceResponseDto getPersonalWalletBalance(
    ...
    return new PersonalWalletBalanceResponseDto(
        customerId, personalWallet.getWalletId(), storeBalances);
  }
  ...
  public GroupWalletBalanceResponseDto getGroupWalletBalance(
    ...
    return new GroupWalletBalanceResponseDto(
        groupId, groupWallet.getWalletId(), group.getGroupName(), storeBalances);
  }
  ...
  public WalletStoreDetailResponseDto getPersonalWalletStoreDetail(
    ...
    WalletStoreBalance balance =
        balanceRepository
            .findByWalletAndStore(personalWallet, store)
            .orElse(
                WalletStoreBalance.builder()
    ...
    Page<Transaction> transactions =
        transactionRepository.findValidTransactionsByCustomerAndStore(
            customerId, storeId, pageable);

    // 5. Transaction을 DTO로 변환
    Page<WalletStoreTransactionDetailDto> transactionDtos =
        transactions.map(WalletStoreTransactionDetailDto::from);
  ...
  public WalletStoreDetailResponseDto getGroupWalletStoreDetail(
    ...
    WalletStoreBalance balance =
        balanceRepository
            .findByWalletAndStore(groupWallet, store)
            .orElse(
                WalletStoreBalance.builder().wallet(groupWallet).store(store).balance(0L).build());

    // 5. 거래내역 조회 (페이징) - 간소화
    Page<Transaction> transactions =
        transactionRepository.findValidTransactionsByGroupAndStore(groupId, storeId, pageable);
  ...
  private void validOwnershipAndMembership(
      Long userId, Long groupId, Wallet individual, Wallet group) {
    if (individual.getCustomer() == null
        || !individual.getCustomer().getCustomerId().equals(userId))
      throw new CustomException(ErrorCode.BAD_REQUEST);
