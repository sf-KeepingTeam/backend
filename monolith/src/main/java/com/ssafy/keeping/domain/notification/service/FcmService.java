...
public class FcmService {
  ...
  public void registerCustomerToken(Long customerId, String token) {
    ...
    FcmToken fcmToken = fcmTokenRepository.findByCustomerIdAndToken(customerId, token).orElse(null);

    if (fcmToken != null) {
      fcmToken.updateToken(token);
      log.info("기존 고객 FCM 토큰 업데이트 완료 - 고객ID: {}", customerId);
    } else {
      fcmToken = FcmToken.builder().token(token).customer(customer).build();
  ...
  public void registerOwnerToken(Long ownerId, String token) {
    ...
    FcmToken fcmToken = fcmTokenRepository.findByOwnerIdAndToken(ownerId, token).orElse(null);

    if (fcmToken != null) {
      fcmToken.updateToken(token);
      log.info("기존 점주 FCM 토큰 업데이트 완료 - 점주ID: {}", ownerId);
    } else {
      fcmToken = FcmToken.builder().token(token).owner(owner).build();
      fcmTokenRepository.save(fcmToken);
  ...
  public void sendToCustomer(
      Long customerId, NotificationType type, String title, String body, Map<String, String> data) {
  ...
  public void sendToOwner(
      Long ownerId, NotificationType type, String title, String body, Map<String, String> data) {
