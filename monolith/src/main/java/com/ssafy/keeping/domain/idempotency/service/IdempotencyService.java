...
public class IdempotencyService {
  ...
  /**
   * 멱등키 '선점 또는 로드' - 같은 범위(ActorType+ActorId+Path+KeyUuid)에 기존 레코드가 있으면 그대로 리턴 - 없으면 IN_PROGRESS로
   * 새로 만들고 리턴
   */
  @Transactional
  public IdemBegin beginOrLoad(
    ...
    Optional<IdempotencyKey> existing =
        idempotencyKeyRepository.findByActorTypeAndActorIdAndPathAndKeyUuid(
            actorType, actorId, path, keyUuid);
  ...
  /** 요청 본문(bodyHash) 충돌 여부 확인 - 동일 멱등키로 처음 요청과 다른 본문이 오면 true(충돌) */
  ...
  /** 처리 완료 기록(DONE) + 응답 스냅샷 저장 - 이후 동일 키로 재호출 시 이 응답을 재생(replay) 가능 */
  @Transactional
  public void complete(
      IdempotencyKey row, int httpStatus, Object responseBody, UUID resourcePublicId) {
  ...
  /**
   * SHA-256 유틸 (정규화된 요청 바디 문자열 → 32바이트 해시) - 반드시 먼저 원문 정규화가 필요!! - JSON 키 순서, 공백, 기본값 누락 등으로 같은 의미의
   * 요청이 다른 바이트열이 되기 쉬움.
   */
  ...
  @Transactional
  public void completeWithoutSnapshot(IdempotencyKey row, int httpStatus, UUID intentPublicId) {
    row.setStatus(IdemStatus.DONE);
    row.setHttpStatus(httpStatus);
