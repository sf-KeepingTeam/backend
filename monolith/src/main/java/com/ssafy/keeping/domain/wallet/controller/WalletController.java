...
public class WalletController {
  ...
  public ResponseEntity<ApiResponse<AvailablePointResponseDto>> getReclaimable(
      ...
      @AuthenticationPrincipal UserPrincipal principal) {
    Long customerId = principal.id();
    AvailablePointResponseDto dto =
        walletService.getReclaimablePoints(walletId, storeId, customerId);
  ...
  public ResponseEntity<ApiResponse<PersonalWalletBalanceResponseDto>> getPersonalWalletBalance(
    ...
    PersonalWalletBalanceResponseDto dto =
        walletService.getPersonalWalletBalance(customerId, pageable);
    return ResponseEntity.ok(
        ApiResponse.success("개인 지갑 잔액 조회에 성공했습니다.", HttpStatus.OK.value(), dto));
  }
  ...
  public ResponseEntity<ApiResponse<GroupWalletBalanceResponseDto>> getGroupWalletBalance(
    ...
    Long customerId = principal.id();
    Pageable pageable = PageRequest.of(page, size);
    GroupWalletBalanceResponseDto dto =
        walletService.getGroupWalletBalance(groupId, customerId, pageable);
    return ResponseEntity.ok(
        ApiResponse.success("모임 지갑 잔액 조회에 성공했습니다.", HttpStatus.OK.value(), dto));
  }
  ...
  public ResponseEntity<ApiResponse<WalletStoreDetailResponseDto>> getPersonalWalletStoreDetail(
    ...
    Long customerId = principal.id();
    Pageable pageable = PageRequest.of(page, size);
    WalletStoreDetailResponseDto dto =
        walletService.getPersonalWalletStoreDetail(customerId, storeId, pageable);
    return ResponseEntity.ok(
        ApiResponse.success("개인 지갑 가게별 상세 정보 조회에 성공했습니다.", HttpStatus.OK.value(), dto));
  }
  ...
  public ResponseEntity<ApiResponse<WalletStoreDetailResponseDto>> getGroupWalletStoreDetail(
    ...
    Pageable pageable = PageRequest.of(page, size);
    WalletStoreDetailResponseDto dto =
        walletService.getGroupWalletStoreDetail(groupId, customerId, storeId, pageable);
    return ResponseEntity.ok(
        ApiResponse.success("모임 지갑 가게별 상세 정보 조회에 성공했습니다.", HttpStatus.OK.value(), dto));
  }
  ...
  public ResponseEntity<ApiResponse<BothWalletBalanceResponseDto>> getBothWalletBalance(
    ...
    return ResponseEntity.ok(
        ApiResponse.success("개인 지갑 및 모임 지갑 조회에 성공했습니다.", HttpStatus.OK.value(), dto));
  }
}
