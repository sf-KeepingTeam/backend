...
public class GroupService {
  ...
  public GroupResponseDto createGroup(Long groupLeaderId, GroupRequestDto requestDto) {
    ...
    String groupDescription =
        (requestDto.getGroupDescription() == null || requestDto.getGroupDescription().isBlank())
    ...
    groupMemberRepository.save(
        GroupMember.builder().group(saved).user(customer).leader(true).build());
  ...
  public GroupResponseDto getGroup(Long groupId, Long userId) {
    ...
    boolean isGroupMember = groupMemberRepository.existsMember(groupId, userId);
    if (!isGroupMember) throw new CustomException(ErrorCode.ONLY_GROUP_MEMBER);
  ...
  public GroupResponseDto editGroup(Long groupId, Long customerId, GroupEditRequestDto requestDto) {
    ...
    boolean isGroupLeader = groupMemberRepository.existsLeader(groupId, customerId);
    if (!isGroupLeader) throw new CustomException(ErrorCode.ONLY_GROUP_LEADER);
  ...
  public List<GroupMemberResponseDto> getGroupMembers(Long groupId, Long customerId) {
    ...
    boolean isGroupMember = groupMemberRepository.existsMember(groupId, customerId);
  ...
  public void createGroupAddRequest(Long groupId, Long customerId) {
    ...
    boolean isGroupMember = groupMemberRepository.existsMember(groupId, customerId);
    if (isGroupMember) throw new CustomException(ErrorCode.ALREADY_GROUP_MEMBER);

    Customer customer = validCustomer(customerId);

    boolean alreadyRequest =
        groupAddRequestRepository.existsRequest(groupId, customerId, RequestStatus.PENDING);

    if (alreadyRequest) throw new CustomException(ErrorCode.ALREADY_GROUP_REQUEST);

    groupAddRequestRepository.save(GroupAddRequest.builder().user(customer).group(group).build());

    Long leaderId =
        groupMemberRepository
            .findLeaderId(groupId)
            .orElseThrow(() -> new CustomException(ErrorCode.GROUP_LEADER_NOT_FOUND));

    afterCommit(
        () ->
            notificationService.sendToCustomer(
                leaderId, NotificationType.GROUP_JOIN_REQUEST, "새 가입 요청이 도착했습니다."));
  }
  ...
  @Transactional
  public AddRequestResponseDto updateAddRequestStatus(
      Long groupId, Long customerId, @Valid AddRequestDecisionDto request) {
    Group group = validGroup(groupId);

    Long groupAddRequestId = request.getGroupAddRequestId();

    GroupAddRequest groupAddRequest =
        groupAddRequestRepository
            .findById(groupAddRequestId)
            .orElseThrow(() -> new CustomException(ErrorCode.ADD_REQUEST_NOT_FOUND));
    ...
    RequestStatus changeStatus =
        request.getIsAccept() == Boolean.TRUE ? RequestStatus.ACCEPT : RequestStatus.REJECT;
    ...
    if (changeStatus == RequestStatus.ACCEPT) {
      ...
      if (!groupMemberRepository.existsMember(groupId, requesterId)) {
        groupMemberRepository.save(
            GroupMember.builder().group(group).leader(false).user(requester).build());
      }
    }
    ...
    return new AddRequestResponseDto(
        groupAddRequest.getGroupAddRequestId(),
        groupAddRequest.getUser().getName(),
        groupAddRequest.getRequestStatus());
  }

  @Transactional
  public GroupResponseDto createGroupMember(
      Long groupId, Long userId, GroupEntranceRequestDto requestDto) {
    Group group = validGroup(groupId);
    ...
    groupMemberRepository.save(GroupMember.builder().group(group).leader(false).user(user).build());
  ...
  @Transactional
  public GroupLeaderChangeResponseDto changeGroupLeader(
      Long groupId, Long userId, @Valid GroupLeaderChangeRequestDto requestDto) {
    validGroup(groupId);
    ...
    if (!originGroupLeader.isLeader()) throw new CustomException(ErrorCode.ONLY_GROUP_LEADER);

    if (!originGroupLeader.changeLeader(false) || !newGroupLeader.changeLeader(true)) {
      throw new CustomException(ErrorCode.BAD_REQUEST);
    }
  ...
  public void expelMember(Long groupId, Long leaderId, Long targetCustomerId) {
    ...
    afterCommit(
        () -> {
          notificationService.sendToCustomer(
              targetCustomerId,
              NotificationType.MEMBER_EXPELLED,
  ...
  public GroupLeaveResponseDto leaveGroup(Long groupId, Long customerId) {
    ...
    return new GroupLeaveResponseDto(
        groupId, customerId, refunded, indivBalance, LocalDateTime.now());
  }
  ...
  public GroupDisbandResponseDto disbandGroup(Long groupId, Long leaderId) {
    ...
    Map<Long, Long> refundedByMember =
        walletService.settleAllMembersShare(groupId, memberIds); // 변경
    ...
    afterCommit(
        () ->
            refundedByMember.forEach(
                (cid, amt) ->
                    notificationService.sendToCustomer(
                        cid, NotificationType.GROUP_DISBANDED, "모임 해체. 환급 " + amt + "P 완료.")));

    return new GroupDisbandResponseDto(
        groupId, memberIds.size(), totalRefunded, refundedByMember, LocalDateTime.now());
  }

  /** ===============validate method============= Group, GroupMember, Customer, GroupWallet 검증 */
