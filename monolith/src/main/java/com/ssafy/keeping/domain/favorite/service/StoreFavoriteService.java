...
public class StoreFavoriteService {
  ...
  public FavoriteToggleResponseDto toggleFavorite(Long customerId, Long storeId) {
    ...
    if (existingFavorite.isPresent()) {
      ...
      if (favorite.isActive()) {
        favorite.cancel();
        return new FavoriteToggleResponseDto(customerId, storeId, false, favorite.getFavoriteId());
      } else {
        favorite.reactivate();
        return new FavoriteToggleResponseDto(customerId, storeId, true, favorite.getFavoriteId());
      }
    } else {
      StoreFavorite newFavorite =
          StoreFavorite.builder().customer(customer).store(store).active(true).build();

      StoreFavorite savedFavorite = storeFavoriteRepository.save(newFavorite);

      return new FavoriteToggleResponseDto(
          customerId, storeId, true, savedFavorite.getFavoriteId());
    }
  }
  ...
  public StoreFavoriteResponseDto getFavoriteStores(Long customerId, Pageable pageable) {
    ...
    Page<StoreFavorite> favoritePage =
        storeFavoriteRepository.findByCustomerAndActiveTrueOrderByFavoritedAtDesc(
            customer, pageable);

    Page<SimpleFavoriteDto> favoriteStores =
        favoritePage.map(
            sf ->
                new SimpleFavoriteDto(
                    sf.getFavoriteId(), sf.getStore().getStoreId(), sf.getFavoritedAt()));

    long totalCount = storeFavoriteRepository.countByCustomerAndActiveTrue(customer);

    return new StoreFavoriteResponseDto(customerId, totalCount, favoriteStores);
  }
