...
public class OwnerMenuCategoryController {
  ...
  public ResponseEntity<ApiResponse<MenuCategoryResponseDto>> createMenuCategory(
    ...
    MenuCategoryResponseDto dto =
        menuCategoryService.createMenuCategory(ownerId, storeId, requestDto);
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(ApiResponse.success("메뉴 카테고리가 등록되었습니다", HttpStatus.CREATED.value(), dto));
  }
  ...
  public ResponseEntity<ApiResponse<List<MenuCategoryResponseDto>>> getAllMenuCategory(
    ...
    return ResponseEntity.ok(
        ApiResponse.success("해당 가게의 메뉴 카테고리(대분류)가 전체 조회되었습니다.", HttpStatus.OK.value(), dtoList));
  }
  ...
  public ResponseEntity<ApiResponse<MenuCategoryResponseDto>> editMenuCategory(
    ...
    Long ownerId = principal.id();
    MenuCategoryResponseDto dto =
        menuCategoryService.editMenuCategory(ownerId, storeId, categoryId, requestDto);
