...
public class TossPaymentGateway implements PaymentGateway {
  ...
  public PaymentResult processPayment(PaymentRequest request) {
    log.info(
        "[TossGateway] 결제 처리 시작 - orderId: {}, amount: {}",
        request.getOrderId(),
        request.getAmount());
    ...
    if (tossResponse.isSuccess()) {
      LocalDateTime approvedAt =
          tossResponse.getApprovedAt() != null
              ? tossResponse
                  .getApprovedAt()
      ...
      PaymentResult.PaymentResultBuilder resultBuilder =
      ...
      return PaymentResult.failure(tossResponse.getCode(), tossResponse.getMessage());
    }
  }
  ...
  public CancelResult cancelPayment(CancelRequest request) {
    log.info(
        "[TossGateway] 결제 취소 시작 - paymentKey: {}, reason: {}",
        request.getPaymentKey(),
        request.getCancelReason());

    // 공통 요청 → 토스 요청으로 변환
    TossCancelRequest.TossCancelRequestBuilder tossRequestBuilder =
    ...
    TossCancelResponse tossResponse =
        tossPaymentClient.cancelPayment(request.getPaymentKey(), tossRequest);
    ...
    if (tossResponse.isSuccess()) {
      ...
      LocalDateTime canceledAt =
          latestCancel != null && latestCancel.getCanceledAt() != null
              ? latestCancel
                  .getCanceledAt()
                  .atZoneSameInstant(ZoneId.of("Asia/Seoul"))
                  .toLocalDateTime()
      ...
      Long cancelAmount =
          latestCancel != null ? latestCancel.getCancelAmount() : request.getCancelAmount();
      ...
      return CancelResult.failure(tossResponse.getCode(), tossResponse.getMessage());
    }
  }
