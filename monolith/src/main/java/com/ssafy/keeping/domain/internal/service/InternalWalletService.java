...
public class InternalWalletService {
  ...
  public WalletBalanceResponse getBalance(Long walletId, Long storeId) {
    WalletStoreBalance balance =
        balanceRepository.findByWalletIdAndStoreId(walletId, storeId).orElse(null);

    BigDecimal balanceAmount =
        balance != null ? BigDecimal.valueOf(balance.getBalance()) : BigDecimal.ZERO;
  ...
  /** 자금 캡처 (멱등성 보장) Idempotency-Key를 통해 네트워크 재시도 시 중복 결제 방지 */
  @Transactional
  public IdempotentResult<FundsResponse> captureIdempotent(
      FundsCaptureRequest request, String idempotencyKeyHeader) {
    ...
    IdemBegin begin =
        idempotencyService.beginOrLoad(
            IdemActorType.SYSTEM, request.getCustomerId(), "POST", path, keyUuid, bodyHash);
  ...
  public FundsResponse capture(FundsCaptureRequest request) {
    ...
    WalletStoreBalance balance;
    try {
      balance = balanceRepository.lockByWalletIdAndStoreId(walletId, storeId).orElse(null);
    ...
    if (balance == null || balance.getBalance() < amount) {
      log.warn(
          ...
          storeId,
          amount,
          balance != null ? balance.getBalance() : 0);
    ...
    List<WalletStoreLot> spendableLots =
        walletStoreLotRepository.findSpendableLots(walletId, storeId, now);
    long lotLeft = amount;
    ...
    log.info(
        "자금 캡처 완료: walletId={}, storeId={}, amount={}, txId={}",
        walletId,
        storeId,
        amount,
  ...
  public IdempotentResult<RefundResponse> processRefundIdempotent(
      Long walletId, RefundRequest request, String idempotencyKeyHeader) {
