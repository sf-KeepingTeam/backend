...
/** 충전 Phase A (검증·지갑 확보) + Phase B (적립) 전담 빈. PrepaymentService(saga 오케스트레이터)에서 호출. */
@Service
...
public class PrepaymentConfirmService {
  ...
  /** Phase A: 예약 비관락 조회 + 검증 + 멱등성(COMPLETED→replay) + 지갑 확보. 커밋 후 비관락 해제 — 토스 호출 전에 락을 푼다. */
  @Transactional
  public ConfirmPrepareResult prepareConfirm(
    ...
    if (reservation.getStatus() == PaymentReservation.ReservationStatus.COMPLETED) {
      ...
      PrepaymentResponseDto replayDto =
    ...
    log.info(
        "[충전-PhaseA] 검증 완료 - orderId: {}, amount: {}",
        request.getOrderId(),
  ...
  public PrepaymentResponseDto creditPoints(
    ...
    // 보너스 계산
    Optional<ChargeBonus> chargeBonusOpt =
        chargeBonusService.findChargeBonusByAmount(store.getStoreId(), paymentAmount);
    ...
    WalletStoreBalance balance =
        walletStoreBalanceRepository
            ...
            .orElseGet(
                () -> WalletStoreBalance.builder().wallet(wallet).store(store).balance(0L).build());
    ...
    log.info(
        "[충전-PhaseB] 적립 완료 - orderId: {}, 결제: {}원, 보너스: {}원, 총: {}P",
        orderId,
        paymentAmount,
        bonusAmount,
        totalPoints);
  ...
  private Wallet findOrCreateIndividualWallet(
      com.ssafy.keeping.domain.user.customer.model.Customer customer) {
