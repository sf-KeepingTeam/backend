...
/** Menu 캐시 저장소 Push 기반 캐싱: 모놀리스에서 Webhook으로 갱신 단조 version 비교로 재정렬·중복 방어 + tombstone으로 삭제 부활 방지 */
@Slf4j
@Repository
...
public class MenuCacheRepository {
  ...
  /**
   * Menu 캐시 저장 (version 비교 — cache warming용, 기존 version보다 높을 때만 저장) warming 데이터는 실제 version을 가져
   * 올바르게 저장됨.
   */
  ...
  /**
   * version이 저장된 version보다 클 때만 save. // TODO: 멀티 스레드 환경에서 version 비교+갱신 원자성 보장을 위해 Redis Lua 스크립트
   * 도입 권장
   */
  public void saveIfNewer(Long menuId, MenuResponse menu, long version) {
    ...
    if (storedVersion != null && version <= storedVersion) {
      log.debug(
          "Menu 캐시 업데이트 무시 (stale): menuId={}, incoming={}, stored={}",
          menuId,
          version,
          storedVersion);
      return;
  ...
  /**
   * version이 저장된 version보다 클 때만 evict + tombstone. // TODO: 멀티 스레드 환경에서 version 비교+갱신 원자성 보장을 위해
   * Redis Lua 스크립트 도입 권장
   */
  public void evictIfNewer(Long menuId, long version) {
    ...
    if (storedVersion != null && version <= storedVersion) {
      log.debug(
          "Menu 캐시 삭제 무시 (stale): menuId={}, incoming={}, stored={}",
          menuId,
          version,
          storedVersion);
      return;
  ...
  /** 전체 Menu 캐시 저장 (Cache Warming용) 실제 version(updatedAt epoch millis)으로 저장. */
