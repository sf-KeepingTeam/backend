...
public class SlackAlertService implements AlertService {
  ...
  public SlackAlertService(
      @org.springframework.beans.factory.annotation.Qualifier("slackRestTemplate")
  ...
  public void notifyCircuitBreakerOpen(String clientName) {
    String message =
        String.format(
            ...
            clientName, LocalDateTime.now().format(FORMATTER));
    send(message);
  }
    ...
    String message =
        String.format(
            "⚠\uFE0F [RECOVERY_WARNING] 복구 재시도 5회 이상\n"
                ...
                + "• 현재 재시도 횟수: %d\n"
                + "• 발생 시각: %s",
            intent.getIntentId(), retryCount, LocalDateTime.now().format(FORMATTER));
    send(message);
  }
