...
/** Anti-Corruption Layer 모놀리스의 Notification 서비스를 HTTP로 호출 */
@Slf4j
@Component
...
public class NotificationClient {
  ...
  private void sendToCustomerFallback(Long customerId, String type, String content, Throwable t) {
    log.warn(
        "Notification 전송 Fallback (Customer): customerId={}, type={}, error={}",
        customerId,
        type,
        t.getMessage());
  ...
  private void sendToOwnerFallback(Long ownerId, String type, String content, Throwable t) {
    log.warn(
        "Notification 전송 Fallback (Owner): ownerId={}, type={}, error={}",
        ownerId,
        type,
        t.getMessage());
    // 알림 실패는 비즈니스 로직에 영향을 주지 않음 - 무시
