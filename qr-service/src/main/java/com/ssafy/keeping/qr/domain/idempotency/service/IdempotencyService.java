...
public class IdempotencyService {
  ...
  /** IN_PROGRESS 상태로 5분 초과된 레코드 정리 (5분마다 실행) 비정상 종료 등으로 DONE 전이 못한 고아 레코드 삭제 */
  @Scheduled(fixedRate = 300_000)
  @Transactional
  public void cleanupStalledInProgress() {
    LocalDateTime threshold = LocalDateTime.now(clock).minusMinutes(5);
    List<IdempotencyKey> stalled =
        idempotencyKeyRepository.findByStatusAndCreatedAtBefore(IdemStatus.IN_PROGRESS, threshold);
