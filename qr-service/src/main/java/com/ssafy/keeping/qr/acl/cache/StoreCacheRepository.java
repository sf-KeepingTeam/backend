...
public class StoreCacheRepository {
    ...
    /**
     * Store 캐시 저장 (version 비교 — cache warming용)
    ...
    public void saveIfNewer(Long storeId, StoreResponse store, long version) {
        ...
        if (storedVersion != null && version <= storedVersion) {
            log.debug("Store 캐시 업데이트 무시 (stale): storeId={}, incoming={}, stored={}",
                    storeId, version, storedVersion);
            return;
        }
    ...
    /**
     * version이 저장된 version보다 클 때만 evict.
     * 데이터는 삭제하되 tombstone(version 마커)을 유지하여 stale update의 부활을 방지.
     * // TODO: 멀티 스레드 환경에서 version 비교+갱신 원자성 보장을 위해 Redis Lua 스크립트 도입 권장
     */
    public void evictIfNewer(Long storeId, long version) {
        ...
        if (storedVersion != null && version <= storedVersion) {
            log.debug("Store 캐시 삭제 무시 (stale): storeId={}, incoming={}, stored={}",
                    storeId, version, storedVersion);
            return;
        }
