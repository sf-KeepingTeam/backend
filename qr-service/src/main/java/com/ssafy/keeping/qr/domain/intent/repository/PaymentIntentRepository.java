...
public interface PaymentIntentRepository extends JpaRepository<PaymentIntent, Long> {
  ...
  @Modifying
  @Query(
      "UPDATE PaymentIntent pi SET pi.status = 'EXPIRED' WHERE pi.status = 'PENDING' AND pi.expiresAt < :now")
  int expirePendingIntents(@Param("now") LocalDateTime now);

  /** 복구 대상 조회: UNCERTAIN 상태이거나 만료된 PENDING 상태의 Intent retryCount > 10인 건은 이미 최대 재시도 초과이므로 제외 */
  @Query(
      """
