...
6. **Redis에 JSON으로 저장되는 모델에 파생 `isXxx()`/`getXxx()`를 추가할 때는 `@JsonIgnore`를 붙일 것.** Jackson이 getter 모양을 프로퍼티로 인식해 JSON에 없는 필드를 만들고, 역직렬화에서 터진다. 단위 테스트로는 안 잡히고 **Redis 왕복 테스트에서만** 드러난다.
