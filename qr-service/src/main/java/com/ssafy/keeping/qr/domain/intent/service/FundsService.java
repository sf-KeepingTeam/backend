...
public class FundsService {
  ...
  /**
   * 자금 캡처 (잔액 차감 + 거래 내역 생성) PaymentIntent의 publicId를 기반으로 결정적 멱등성 키 생성하여 중복 결제 방지
   *
   * <p>타임아웃/서킷 오픈 시: 1. Intent를 UNCERTAIN 상태로 변경 2. PaymentRecoveryService에 복구 필요 플래그 설정 3. Fast
   * Fail - 즉시 에러 응답
   */
  public FundsResult capture(PaymentIntent intent, List<PaymentIntentItem> items) {
    ...
    try {
      List<FundsCaptureRequest.ItemSnapshot> itemSnapshots =
          items.stream()
              .map(
                  item ->
      ...
      if (isNetworkOrCircuitError(e)) {
        ...
        log.warn(
            "자금 캡처 타임아웃/서킷 오픈 - UNCERTAIN 상태로 변경: intentId={}, reason={}",
            intent.getIntentId(),
  ...
  private void markIntentUncertain(PaymentIntent intent, String reason) {
    try {
      ...
      log.error(
          "Intent UNCERTAIN 상태 변경 실패: intentId={}, error={}", intent.getIntentId(), e.getMessage());
    }
  }
  ...
  /** PaymentIntent 기반 결정적 멱등성 키 생성 동일 intent에 대한 재시도 시 동일한 키가 생성됨 */
