...
/** 캐시 워밍 서비스 애플리케이션 시작 시 모놀리스에서 전체 Store/Menu 데이터를 가져와 캐시에 적재 WRITE_THROUGH 모드에서만 캐시 워밍 실행 */
@Slf4j
@Service
...
public class CacheWarmingService {
  ...
  /** 애플리케이션 시작 시 캐시 워밍 실행 WRITE_THROUGH 모드에서만 실행 */
  @Async
  ...
  private void warmStoreCache() {
    ...
    try {
      ...
      ResponseEntity<List<StoreResponse>> response =
