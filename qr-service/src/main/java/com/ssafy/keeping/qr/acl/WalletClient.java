...
/**
 * Anti-Corruption Layer 모놀리스의 Wallet 서비스를 HTTP로 호출
 *
 * <p>RestTemplate 용도별 분리: - restTemplate: 읽기 작업 (5초 타임아웃) - writeRestTemplate: 쓰기 작업 (3초 Fail-Fast)
...
public class WalletClient {
  ...
  private BigDecimal getBalanceFallback(Long walletId, Long storeId, Throwable t) {
    log.error(
        "Wallet 잔액 조회 Fallback 호출: walletId={}, storeId={}, error={}",
        walletId,
        storeId,
        t.getMessage());
  ...
  /**
   * 자금 캡처 (결제 시 잔액 차감 + 거래 내역 생성) - writeRestTemplate 사용 (3초 Fail-Fast) - 재시도 없음 (maxAttempts: 1) -
  ...
  private FundsResponse captureFallback(
      FundsCaptureRequest request, String idempotencyKey, Throwable t) {
    log.error(
        "자금 캡처 Fallback 호출: walletId={}, storeId={}, amount={}, idempotencyKey={}, error={}",
        request.getWalletId(),
        request.getStoreId(),
        request.getAmount(),
        idempotencyKey,
  ...
  private PaymentCheckResponse checkPaymentFallback(String idempotencyKey, Throwable t) {
    log.error("결제 상태 확인 Fallback 호출: idempotencyKey={}, error={}", idempotencyKey, t.getMessage());
    throw new CustomException(ErrorCode.WALLET_SERVICE_UNAVAILABLE, t);
  }

  /** 환불 처리 - 기존 결제에 대한 환불 - writeRestTemplate 사용 (3초 Fail-Fast) - 재시도 없음 (maxAttempts: 1) */
  ...
  private RefundResponse refundFallback(RefundRequest request, String idempotencyKey, Throwable t) {
    log.error(
        "환불 처리 Fallback 호출: walletId={}, storeId={}, amount={}, idempotencyKey={}, error={}",
        request.getWalletId(),
        request.getStoreId(),
        request.getAmount(),
        idempotencyKey,
  ...
  /**
   * 결제 상태 확인 - 복구용 - recoveryRestTemplate 사용 (10초 타임아웃) - 재시도 허용 (maxAttempts: 3) - 트랜잭션 외부에서 호출해야
   * 함
   */
  ...
  private PaymentCheckResponse checkPaymentForRecoveryFallback(String idempotencyKey, Throwable t) {
    log.error(
        "결제 상태 확인 (복구) Fallback 호출: idempotencyKey={}, error={}", idempotencyKey, t.getMessage());
    throw new CustomException(ErrorCode.WALLET_SERVICE_UNAVAILABLE, t);
  }

  /**
   * 환불 처리 - 복구용 - recoveryRestTemplate 사용 (10초 타임아웃) - 재시도 허용 (maxAttempts: 3) - 트랜잭션 외부에서 호출해야 함
   */
  ...
  public RefundResponse refundForRecovery(RefundRequest request, String idempotencyKey) {
    ...
    try {
      ...
      log.error(
          "환불 4xx 수신 (영구 실패): walletId={}, status={}, body={}",
          request.getWalletId(),
          e.getStatusCode(),
          e.getResponseBodyAsString());
      return RefundResponse.permanentFailed(e.getStatusCode().value(), e.getResponseBodyAsString());
    }
  }

  private RefundResponse refundForRecoveryFallback(
      RefundRequest request, String idempotencyKey, Throwable t) {
    log.error(
        "환불 처리 (복구) Fallback 호출: walletId={}, storeId={}, amount={}, idempotencyKey={}, error={}",
        request.getWalletId(),
        request.getStoreId(),
        request.getAmount(),
        idempotencyKey,
