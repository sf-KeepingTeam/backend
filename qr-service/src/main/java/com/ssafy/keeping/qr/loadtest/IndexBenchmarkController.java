...
/**
 * 인덱스 성능 벤치마크용 테스트 컨트롤러 loadtest.backdoor.enabled=true 일 때만 활성화
 *
 * <p>테스트 대상: - 쓰기: PaymentIntent INSERT (인덱스가 쓰기 성능에 미치는 영향) - 읽기: findRecoveryTargets 쿼리
 * (idx_recovery 인덱스 사용)
 */
@Slf4j
@RestController
...
public class IndexBenchmarkController {
  ...
  /**
   * 쓰기 테스트: PaymentIntent 생성 POST /loadtest/benchmark/write
   *
   * <p>테스트 목적: 인덱스가 INSERT 성능에 미치는 영향 측정 - idx_recovery (status, expires_at, created_at) -
   * idx_status_expires (status, expires_at) - idx_store_status (store_id, status) -
  ...
  /**
   * 읽기 테스트: findRecoveryTargets 쿼리 호출 GET /loadtest/benchmark/read
   *
   * <p>테스트 목적: idx_recovery 인덱스가 SELECT 성능에 미치는 영향 측정 쿼리: WHERE (status = 'UNCERTAIN' OR (status =
   * 'PENDING' AND expires_at < now)) AND created_at > since ORDER BY created_at ASC
   */
  @GetMapping("/read")
  public ResponseEntity<Map<String, Object>> readTest(@RequestParam(defaultValue = "7") int days) {
    LocalDateTime now = LocalDateTime.now();
  ...
  /** 벌크 쓰기 테스트: 여러 PaymentIntent 한번에 생성 POST /loadtest/benchmark/write-bulk */
  ...
  public ResponseEntity<Map<String, Object>> writeBulkTest(@RequestBody BulkWriteRequest request) {
    ...
    return ResponseEntity.ok(Map.of("success", true, "createdCount", created));
  }

  /** 테스트 데이터 정리 DELETE /loadtest/benchmark/cleanup */
  @DeleteMapping("/cleanup")
  public ResponseEntity<Map<String, Object>> cleanup(
      @RequestParam(defaultValue = "bench-") String prefix) {
    // qrTokenId가 prefix로 시작하는 것만 삭제
    List<PaymentIntent> toDelete =
        paymentIntentRepository.findAll().stream()
            .filter(pi -> pi.getQrTokenId() != null && pi.getQrTokenId().startsWith(prefix))
            .toList();

    paymentIntentRepository.deleteAll(toDelete);

    return ResponseEntity.ok(Map.of("success", true, "deletedCount", toDelete.size()));
  }
  ...
  public record BulkWriteRequest(
      Long customerId, Long walletId, Long storeId, Long amount, String status, int count) {}
}
