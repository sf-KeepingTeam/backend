 ...
 *
 * <p>이 테스트의 Stub 정의는 Monolith의 Contract와 동일해야 합니다. Monolith가 Contract를 변경하면 이 테스트의 Stub도 함께 변경해야
 * 합니다.
 *
 * <p>Contract 파일 위치:
 * backend/monolith/src/test/resources/contracts/internal/shouldReturnStoreById.groovy
 *
 * <p>[계약 위반 감지 흐름] 1. Monolith가 API 응답 변경 2. Monolith Contract Test 실패 → 수정 필요 3. 수정 후 QR Service
 * 테스트도 함께 수정 4. 둘 다 통과해야 배포 가능!
 */
class StoreClientContractTest {
   ...
   *
   * <p>Monolith Contract 변경 시 이 Stub도 함께 변경 필요!
   */
  @Test
  ...
  void shouldReturnStoreById() {
    ...
    stubFor(
        get(urlEqualTo("/internal/stores/1"))
